...
| T3 `COMPLEX_IMPLEMENTATION_OR_REPAIR` | broad-but-bounded implementation, large refactors, complex fail-closed work, forensic debug, P1/P2 repair | Opus 4.8 xhigh / Terra xhigh | Explicit invariants and regression proof; no token shortcut |
...
Selection order (lowest capable lane): Luna for mechanics → runtime-proven Sonnet 5 or Terra for bounded
work → Opus for broad/complex local work → Sol only for qualifying protected T4 on a narrow packet.
Sonnet 5 requires runtime proof of availability/identity before any routing; fallback Terra (bounded) /
Opus (broad). If `EXACT_MODEL_REQUIRED=true`, requested/actual mismatch stops. Never claim
unavailable-model quality. Model selection does not prove safety, repo state, or results. No Fable lane is
active; historical Fable contracts stay archived in `fable_exit_contract_index.md` and are never routing.
