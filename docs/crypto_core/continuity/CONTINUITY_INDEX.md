...
Every fact below BELONGS in ephemeral state, and writing one into a durable surface is a defect:
...
What the deterministic scan CATCHES is narrower than that list, and this file does not claim
otherwise. The durable-surface scan in `scripts/crypto_core/validate_agent_os_v2.py` rejects a fact
from the list ONLY when it is written in one of the exact bounded syntactic forms fixed by
`DURABLE_STATE_CLAIM_BOUNDARY` in `docs/crypto_core/agent_os_v2.md` section 15: a qualifying commit
or tree hash token, a `PR #<n>` pin, a `main @ <hash>` pin, or an ASSIGNMENT to a field registered in
`VOLATILE_STATE_FIELDS`.

The same fact written as ordinary English carries no registered field name and no literal pin form,
so the scan does not detect it — and this file does not promise that it will. Arbitrary
natural-language current-state prose, and semantic contradiction generally, remain the
INDEPENDENT SEMANTIC AUDIT's responsibility. Such a sentence in a durable surface is still a defect;
it is simply not a deterministically caught one. Broadening the scanner to chase English is an
explicit anti-pattern and a `ROOT_CAUSE_MODE` trigger, not the fix.
...
- A rejected or frozen candidate's branch, bytes or handoff being read as canonical state — it is historical
  evidence only.
