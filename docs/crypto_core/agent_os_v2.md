...
only in a role that cannot apply it. `NON_APPLYING` is the one role, and the validator rejects the
surface the moment its front matter declares `alwaysApply: true`. Format: `- <path> :: NON_APPLYING`.
