...
`LANE_CAPABILITY` declares each lane's identity and which mutation authorities that lane may be
trusted with. It is the other half of the same question and it is deliberately NOT a copy of the
routing matrix: the family contract says what the WORK is, this says who may be asked to do that
kind of work, and the matrix pairs them. Without it a family kept its correct authority while the
work moved to a lane that should never carry it — `T3A HEAVY_MUTATION` routed to `GPT-5.6 Luna` was
accepted, as was a model id that belongs to no lane at all. A lane absent from this block is not a
routable lane. Format: `- <LANE> :: <MODEL_ID> :: <MUTATION_AUTHORITY>[,<MUTATION_AUTHORITY>]`, with
`-` where the control plane compares no API identity.

<!-- LANE_CAPABILITY_BEGIN -->
- Claude Opus 5 :: claude-opus-5 :: HEAVY_MUTATION,CAPABILITY_CRITICAL_MUTATION,READ_ONLY
- Claude Sonnet 5 :: claude-sonnet-5 :: GOVERNED_CLOSEOUT,BOUNDED_MUTATION
- Codex GPT-5.6 Sol :: gpt-5.6-sol :: BOUNDED_MUTATION,READ_ONLY
- GPT-5.6 Terra :: - :: BOUNDED_MUTATION
- GPT-5.6 Luna :: - :: MECHANICAL_ONLY,GOVERNED_CLOSEOUT
- GPT-6 Astra :: gpt-6-astra :: READ_ONLY
- Deep Research :: - :: READ_ONLY
<!-- LANE_CAPABILITY_END -->
...
- **T3C `REVIEW`** — read-only review and bug discovery, on Claude Opus 5 or Codex GPT-5.6 Sol,
  chosen by the independence rule in section 3.4: `medium` for focused
  ...
  creates a SEPARATE read-only `CLASS_C_CROSS_CONTRACT` task for the protected gate (section 3.4).
...
### 3.3 Intent-first routing and the separate protected gate
...
- 2026-09 — GPT-6 Astra became the canonical protected frontier lane and, for a short period, also
  ...
  valid historical evidence at their original dates; the ACTIVE disposition is section 3.4.
