...
- **T3C `REVIEW`** — read-only review and bug discovery, on Claude Opus 5: `medium` for focused
  ...
  multi-boundary or cross-layer review. `max` is ILLEGAL for T3C. A protected trigger escalates the
  work to T4, not to T3B. Any resulting fix is a separate, explicitly created task. A review by the
  same session that implemented the work is `SELF_AUDIT_ONLY_NOT_INDEPENDENT`.
...
6. `checkout_ref_override`: when the workflow checks out an explicit ref, the default merge-revision
   relation does not apply. The controller must then prove `actual_checkout_revision` from
   authenticated run evidence; if it cannot, the result is `SAFETY_BLOCKER`. The repository validator
   does NOT parse workflow semantics to establish this.
