...
The default size of every PR is `LARGEST_SAFE_SEMANTIC_CLOSURE`, and `MAX_SAFE_PR` names that default.
...
ROUTE: T3C | REVIEW | Claude Opus 5 | claude-opus-5 | medium | READ_ONLY
ROUTE: T3C | REVIEW | Claude Opus 5 | claude-opus-5 | high | READ_ONLY
ROUTE: T3C | REVIEW | Claude Opus 5 | claude-opus-5 | xhigh | READ_ONLY
...
ROUTE: T3D | ARCHITECTURE | Claude Opus 5 | claude-opus-5 | high | READ_ONLY
ROUTE: T3D | ARCHITECTURE | Claude Opus 5 | claude-opus-5 | xhigh | READ_ONLY
...
`FAMILY_SEMANTIC_CONTRACT` declares what each routing family legally IS. The routing matrix above
declares HOW a family is routed — which lane, which model, which effort — and this block declares
what that family is permitted to own and to do. Intent ownership alone was not enough: a family's
mutation authority was only ever checked for INTERNAL consistency, so replacing T0's sole
`MECHANICAL_ONLY` with `HEAVY_MUTATION` left every row agreeing with every other row and the whole
control plane still reported PASS. "All T0 rows agree with each other" is not "T0's authority is the
authority T0 is allowed to have". Each row pins the legal intents, the ONE legal mutation authority
and the exact legal effort set, and every column is checked against the matrix in BOTH directions so
neither can drift. Format: `- <CLASS> :: <INTENT>[,<INTENT>] :: <MUTATION_AUTHORITY> :: <EFFORT>[,<EFFORT>]`.

<!-- FAMILY_SEMANTIC_CONTRACT_BEGIN -->
- T0 :: STATUS :: MECHANICAL_ONLY :: low
- T1 :: CLOSEOUT,BOUNDED_READ :: GOVERNED_CLOSEOUT :: low
- T2 :: IMPLEMENTATION,REPAIR :: BOUNDED_MUTATION :: medium
- T3A :: IMPLEMENTATION,REPAIR :: HEAVY_MUTATION :: xhigh
- T3B :: IMPLEMENTATION,REPAIR :: CAPABILITY_CRITICAL_MUTATION :: max
- T3C :: REVIEW :: READ_ONLY :: medium,high,xhigh
- T3D :: ARCHITECTURE :: READ_ONLY :: high,xhigh,max
- T3E :: PROMPT_ARCHITECTURE :: READ_ONLY :: high,xhigh,max
- T4 :: CLASS_C_CROSS_CONTRACT :: READ_ONLY :: xhigh,max
- XR :: EXTERNAL_RESEARCH :: READ_ONLY :: -
<!-- FAMILY_SEMANTIC_CONTRACT_END -->

`LANE_CAPABILITY` declares each lane's identity and the exact ROUTE FAMILIES that lane may ever
carry. It is the other half of the same question and it is deliberately NOT a copy of the routing
matrix: the family contract says what the WORK is, this says who may be asked to do that kind of
work, and the matrix pairs them. It repeats no intent, no effort and no mutation authority — it
answers only "which family may this lane ever carry?".

Duty is the boundary, NOT mutation authority. An earlier revision listed authorities here, and
`READ_ONLY` conflates five completely different duties — review, architecture, prompt architecture,
protected Class-C and external research — so a lane trusted for one of them was silently trusted for
all five. That is how `XR EXTERNAL_RESEARCH` could be routed to the protected lane, and how the
protected lane could be handed architecture work: every one of those routes is read-only, and
read-only was the whole test. Naming the families closes it, because a family is exactly the thing a
lane is or is not trusted with.

A lane absent from this block is not a routable lane, and a family absent from a lane's row is a
family that lane may never carry. Format:
`- <LANE> :: <MODEL_ID> :: <ROUTE_FAMILY>[,<ROUTE_FAMILY>]`, with `-` where the control plane
compares no API identity.

<!-- LANE_CAPABILITY_BEGIN -->
- Claude Opus 5 :: claude-opus-5 :: T3A,T3B,T3C,T3D,T3E
- Claude Sonnet 5 :: claude-sonnet-5 :: T1,T2
- Codex GPT-5.6 Sol :: gpt-5.6-sol :: T2,T3C
- GPT-5.6 Terra :: - :: T2
- GPT-5.6 Luna :: - :: T0,T1
- GPT-6 Astra :: gpt-6-astra :: T4
- Deep Research :: - :: XR
<!-- LANE_CAPABILITY_END -->
...
- **T2 `BOUNDED_IMPLEMENTATION`** — exact-file deterministic slices, narrow docs, config-only
  changes, mechanical fixtures and tests, obvious localized repair, PR-body corrections. Terra,
  Codex GPT-5.6 Sol or Sonnet 5 according to environment and task need. Never protected
  trust-boundary work.
...
- **T3C `REVIEW`** — read-only review and bug discovery, on Claude Opus 5 or Codex GPT-5.6 Sol,
  chosen by the independence rule in section 3.5: `medium` for focused
  review, `high` for broad, security or post-failure review, `xhigh` for protocol, cryptographic,
  multi-boundary or cross-layer review. `max` is ILLEGAL for T3C. A protected trigger does NOT
  convert this task into T4: a `REVIEW` task stays `REVIEW` and stays here, and the controller
  creates a SEPARATE read-only `CLASS_C_CROSS_CONTRACT` task for the protected gate (section 3.5).
  Any resulting fix is likewise a separate, explicitly created task. A review by the same session
  that implemented the work is `SELF_AUDIT_ONLY_NOT_INDEPENDENT`.
- **T3D `ARCHITECTURE`** — read-only on Claude Opus 5; produces a decision, not a diff. The
  controller owns the decision. `high` for normal serious architecture, `xhigh` for interacting
  candidates or substantial multi-invariant architecture, `max` ONLY for readiness or provenance
  architecture, cryptographic boundaries, irreversible sequencing, or an explicit controller critical
  designation. Ultra remains a capability mode only, under section 4.3.
- **T3E `PROMPT_ARCHITECTURE`** — read-only on Claude Opus 5; the controller owns the compiler and
  the final prompt. `high` for normal serious prompt architecture, `xhigh` for protocol,
...
### 3.3 Independence vocabulary

Three states, decided here and nowhere else. A subordinate adapter may state its own host
consequence but never redefines these, because independence is a property of the ROUTE, not of the
surface describing it.

<!-- INDEPENDENCE_VOCABULARY_BEGIN -->
...
<!-- INDEPENDENCE_VOCABULARY_END -->

So Claude Opus 5 reviewing Codex GPT-5.6 Sol's implementation IS ordinary independent review, and so
is Sol reviewing Opus's - that is exactly what the two T3C lanes are for. Opus reviewing Opus, or Sol
reviewing Sol, is self-audit whatever the effort. Neither lane is ever the protected gate. A
subordinate surface that says "no session of this host satisfies an independent audit" is
overbroad: it denies a route canonical doctrine grants.

### 3.4 Intent-first routing and the separate protected gate

`INTENT_FIRST_ROUTING` — the routing matrix is a function OF the task intent. A trigger, a risk
class, an audit outcome and a protected finding are all inputs to WHICH TASKS EXIST, never inputs
that rewrite the intent of a task already created. Saying that review "escalates into" the protected
family reads as though one task changes identity mid-flight; that is what previously let a family be
selected by something other than its intent, and it is retired.

<!-- INTENT_FIRST_ROUTING_BEGIN -->
...
<!-- INTENT_FIRST_ROUTING_END -->

### 3.5 Frontier-lane disposition
...
architecture. That is deliberate and is NOT a protected-lane substitution: those families are
unprotected reasoning work, while Class C stays with the protected lane. A Claude session that
...
Codex GPT-5.6 Sol is the repo-native accelerator for exactly two duties, and it is routed for
exactly those two: bounded clear-spec implementation and repair at T2, and ordinary fresh-context
review at T3C. It is deliberately NOT a general lane — it carries `BOUNDED_MUTATION` and `READ_ONLY`
and nothing else, so it can never take T3A heavy implementation, T3B capability-critical work or T4.
An ordinary Codex review is NOT a Class-C audit, and no lane inherits T4 authority from being useful.
Sol design and audit records produced while Sol WAS the protected lane remain valid historical
evidence at their original dates and are never rewritten, back-dated or relabelled to pretend they
used another model.

`CROSS_FAMILY_REVIEW_PREFERENCE` — T3C legally routes to Claude Opus 5 or to Codex GPT-5.6 Sol, and
the choice is an INDEPENDENCE choice rather than a capacity one. When Opus implemented the work,
prefer Sol for the ordinary review; when Sol implemented it, prefer Opus. A same-model review remains
`SELF_AUDIT_ONLY_NOT_INDEPENDENT` whichever lane it runs on, and neither option is ever the protected
Class-C gate — that stays a separate T4 task on the protected lane. Because Claude remains the
primary deep implementation lane, this also spends scarce Codex capacity where cross-family review
has the highest marginal value instead of on work Claude can already do.

GPT-5.6 Terra is a BOUNDED IMPLEMENTATION lane only. It carries `BOUNDED_MUTATION` and is routed at
T2; it is not an ordinary-review lane and no active surface may assign it review work.
...
```text
MODEL_ID | MODEL_REQUESTED | MODEL_ACTUAL | MODEL_EVIDENCE_SOURCE |
REQUESTED_EFFORT | OBSERVED_EFFORT | EFFORT_EVIDENCE_SOURCE |
THINKING_ACTUAL | CAPABILITY_MODE | HOST_SETTING_RAW | ENVIRONMENT |
CLIENT_VERSION | MODEL_FALLBACK
```

`DIMENSIONAL_RUNTIME_PROOF` — the block has THREE independent evidence dimensions, because model
identity, effort and thinking are observed by different mechanisms and are known independently.
`model_evidence_source` covers the identity dimension only; `effort_evidence_source` covers the
effort dimension only; `thinking_actual` carries its own explicit `ENABLED`/`DISABLED`/`UNKNOWN`.
Collapsing them into one evidence state forced at least one dimension to be misreported: a host that
reports its model but exposes no effort setting had to either invent effort telemetry or throw away a
proven identity. A contradiction in one dimension never erases valid evidence from another, so
`CONTRADICTED` identity alongside `UNKNOWN` effort, and matching identity alongside `CONTRADICTED`
...
`EFFORT_RELATION_PROOF` — effort has exactly the requested/observed shape identity has, and it gets
the same relation. Under an evidence class that asserts execution was OBSERVED — `RUNTIME_TELEMETRY`
or `USER_ATTESTED_UI_SELECTION` — an `OBSERVED_EFFORT` that differs from `REQUESTED_EFFORT` is NOT
ordinary matching evidence: the only honest class is `CONTRADICTED`. `CONTRADICTED` in turn requires
a real conflict: both efforts recorded, and different. The relation names no effort value and holds
across the whole effort enum. It sits entirely inside the effort dimension, so an effort
contradiction never repairs, hides or creates a model-identity contradiction, and the reverse.

`TARGET_BOUND_EXECUTION_PROOF` — both relations above are comparisons, and a comparison needs a
target. An evidence class that asserts execution was OBSERVED — `RUNTIME_TELEMETRY` or
`USER_ATTESTED_UI_SELECTION` — and `CONTRADICTED`, which asserts an observation conflicted with the
request, are valid only when that target exists: a required identity (`MODEL_ID`, or
`MODEL_REQUESTED` where no API identity applies) in the identity dimension, and a `REQUESTED_EFFORT`
in the effort dimension. With no target the observation has nothing to be compared with, so it
proves nothing and the manifest fails closed. `UNKNOWN` or `CONFIGURATION_EVIDENCE_ONLY` with a null
observation remains the honest record and is always available. An effort-mismatch waiver never
manufactures a target.

`EFFORT_MISMATCH_WAIVER` — a human may waive an observed effort mismatch for a SPECIFIC task, and the
manifest records that without erasing the mismatch. `effort_mismatch_waiver` is required in the
runtime block and carries a status and an evidence field:

- `NOT_APPLICABLE` — no effort mismatch is truthfully recorded; evidence is null.
- `NOT_GRANTED` — a `CONTRADICTED` effort mismatch is recorded and no human waived it; evidence is
  null, and the task stops on the mismatch.
- `HUMAN_GRANTED` — a `CONTRADICTED` effort mismatch is recorded and a human waived it for this task;
  evidence is the exact task-specific waiver.

A waiver never changes `REQUESTED_EFFORT` or `OBSERVED_EFFORT`, never restates the actual as the
requested value, and exists only while they differ: `NOT_GRANTED` or `HUMAN_GRANTED` against matching
efforts is rejected, because there is nothing to waive. It is task-specific — there is no generic,
standing, inferred, inherited, controller-originated or agent-originated waiver. It is NOT authority:
it grants no merge, waives no T4 gate, repairs no model mismatch or observed fallback, waives no
safety, security, readiness, live or capital gate, and changes no task intent. It permits continuation
after an observed effort mismatch; it cannot make an ILLEGAL requested effort legal, because route
effort legality lives in the routing matrix and `FAMILY_SEMANTIC_CONTRACT`, which the runtime block
cannot reach. The validator checks that the representation is consistent; it does NOT prove that a
human actually issued the waiver, which live controller evidence must establish before anything
relies on it.
...
never restated as the requested value. The representation is `EFFORT_MISMATCH_WAIVER` in section 4.1.
...
`HOST_UI_LABELS_ARE_LITERAL` — a host label belongs to one host and one model family, and it is recorded exactly
as the operator selected it. The Claude host setting string `xhighultracode`, the Codex GPT-5.6 Sol host label
`Ultra`, and the GPT-6 Astra host labels `Light`, `Medium`, `High`, `Extra High` and `Ultra` are each kept
literal: a label is never mapped across model families, never translated into an API effort enum, and never
normalized to `max`. Where a host exposes `Ultra`, section 4.3 still decides that it is a capability mode and
not an effort.
...
- **Bounded implementation profile (Terra, Sonnet 5).** Bounded implementation and repair, exact
  files, exact tests, no ordinary-review duty and no protected Class-C substitution.
- **Repo-native profile (Codex GPT-5.6 Sol).** Two duties only: bounded clear-spec implementation or
  repair, and ordinary fresh-context review. Give the exact contract and the exact stop. Never a
  protected Class-C substitution and never heavy or capability-critical implementation.
...
- **Execution-environment profile (Work).** Substantial multi-step execution, research and synthesis under
  section 7; never governance authority.
...
`CHATGPT_WORK_LANE` is a first-class controller-routed execution environment and specialist lane for
substantial multi-step work. It is never a governance authority. The controller decides when Work is
useful; the user is never required to design the routing. Where the product surface supports continuing a
conversation in Work, the controller prepares the complete handoff and the user only performs the UI
transition.

Route to Work when it materially helps: cloud browser or computer use; work spanning many files or apps;
persistent long-running workflows; cross-service evidence collection; large research or design synthesis
with a source ledger; artifact production; and supported web interaction. Do not route a simple GitHub
read or a status question to Work.
...
the environment/toolchain + the evidence id. The ephemeral manifest makes the revision half
executable: a completed gate's evidence key must name that manifest's proven head or its tree
(`PROOF_TARGET_BINDINGS`, section 20).
...
- **CLASS_B_CONTROLLER_FIRST** — ordinary bounded product code. The controller maps source, tests and
  ...
  useful. Any uncertainty means the protected Class-C gate is REQUIRED — a separate read-only
  `CLASS_C_CROSS_CONTRACT` task, per section 3.3 — not a change to this task's intent.
...
`CLEAN_REPLACEMENT_CYCLE` — a frozen contract is never repaired again. A clean replacement exists only as the
explicit bounded decision `ROOT_CAUSE_MODE` requires, made by the controller as an architecture decision; without
that decision a replacement is the same blocker under a new name. It starts from exact accepted main and inherits
no commit, no branch lineage and no repair budget from the frozen candidate, whose bytes are evidence and never
authority, and whose blocker families carry forward as the replacement's regression inventory. The replacement
runs the canonical flow once, with at most one consolidated repair, and `FIXED_POINT_STOP` then ends in ACCEPT or
REJECT.

## 14. LARGE_MILESTONE_PROTOCOL

Before implementation of any large capability milestone, the milestone plan states all eight of these:

1. `TARGET_END_STATE` — the end-to-end capability, and the product value it unlocks.
2. `DEPENDENCY_GRAPH` — the implementation dependencies and the true independent boundaries.
3. `SEMANTIC_CONTRACTS` — the contracts to close: trust boundaries, the producer/consumer graph, the
   provenance chain, external assumptions and failure modes.
4. `PROTECTED_BOUNDARIES` — the human gates, protected audits and authorizations the milestone crosses.
5. `EXPECTED_PR_SEQUENCE` — each PR is the largest independently auditable semantic closure; never one
   artifact, module or test merely because it exists.
6. `MERGE_ORDER` — the order in which those PRs may merge, and what each one depends on.
7. `EXIT_CRITERIA` — capability-wide behavioral acceptance, defined before the first PR starts.
8. `ROLLBACK_BOUNDARY` — what each merge can be reverted to without leaving a half-built trust surface.

Two rules hold throughout. `ROOT_CAUSE_TRIGGER` — if one blocker starts multiplying artifacts or phases,
stop implementing and enter `ROOT_CAUSE_MODE`. `PRODUCT_VALUE_CHECK` — every persistent artifact must
answer: what new load-bearing fact does it prove, or what repeated future work does it measurably remove?
...
`ACTIVE_CONTROL_PLANE_SURFACE_SEMANTICS` — there is exactly ONE answer to whether content is active
authority, and every consumer uses it. The typed parse above runs ONCE per surface over the whole text
and yields a line-preserving ACTIVE projection: region markers and everything inside a region are
blank, so diagnostics keep their original line numbers. Every authority reading consumes that
projection and nothing else — the registries and machine-readable blocks of this file, the canonical
declarations, the role and authority-reference markers, the required contract tokens and the routing
matrix span — and so do the volatile-state, model-agnostic, sizing, ratio and global-`max` scans.
Content is therefore exempt from a scan exactly when it cannot carry authority, never one without the
other. Three structural rules keep that true for every present and future reader. Authority syntax is
RESERVED: an exemption region may not contain a block marker, a role or authority-reference marker, a
canonical declaration line or a routing-matrix row, so historical or example prose describes an old
block without reproducing it. An exemption marker may not open or close inside an active authority
block, so no region can remove part of a block from the projection while the block's own markers stay
active. An exemption marker in any other spelling fails, rather than silently leaving intended history
active. Every machine-readable block of this file is required exactly once in the ACTIVE projection,
...
only in its one exact spelling, so a marker in another spelling is refused, never ignored.
...
Fresh proof overrides a stale handoff, and the ephemeral state a fresh session starts from is the latest
bounded ACCEPTED state manifest or handoff. A rejected or frozen candidate - its branch, its bytes or its
handoff - is historical evidence and never becomes canonical state. Archived transcripts are a fallback only for
a material fact that repository doctrine, the continuity index and fresh proof cannot reconstruct. Fresh-chat
acceptance is claimed only after merge, by the postmerge run of section 15.2, and never before.
...
### 17.1 CONTROLLER_EXACT_BYTE_PREMERGE_PROOF_V1

`CI_ENFORCEMENT_IS_NOT_SELF_PROVABLE` — a repository cannot prove from its own bytes that its own CI
step will run. Whether a workflow executes, and whether a failure blocks a merge, are facts about
GitHub's runtime and configuration, not about text in the tree. A validator that parses one
serialization of a mutable workflow is asserting a property it cannot hold, and every attempt to
close that gap by understanding more YAML spellings reopened it in a different spelling.

Authority is therefore split, and no layer may claim another layer's property:

- **GitHub** owns actual workflow execution and which status contexts block a merge.
- **The repository validator** owns only what it can prove from repository inputs. It does NOT claim
  that its own CI step is executed, that GitHub will merge-block on any particular workflow, that a
  required `tests` context identifies a specific workflow file, or that parsing YAML establishes
  runtime semantics.
- **The independent audit** owns the semantic correctness of the exact candidate workflow, validator
  and oracle contract.
- **The controller** owns the fresh premerge re-proof below.
- **The human** owns exact per-PR merge authorization, and nothing else grants it.

Before any merge-readiness verdict, the controller re-proves at the EXACT PR head, from live
evidence: (A) repository, PR, base and head; (B) the base and head workflow inventory; (C) the exact
`.github/workflows/ci.yml` bytes or Git blob identity at that head; (D) whether that workflow differs
from the accepted base; (E) the workflow run tied to that exact head; (F) the workflow path and
source of that run; (G) that the expected `tests` job completed with SUCCESS; (H) that the Agent OS
validator and oracle steps actually existed in that job and completed successfully; (I) that no
load-bearing gate step or job is `skipped`, `neutral` or `cancelled` — none of which is acceptance;
(J) the current branch-protection required checks; (K) the current App and source binding of those
contexts; (L) that the required contexts are terminal-success; (M) that configuration has not drifted
from the evidence the independent audit used; (N) that the audit acceptance applies to this exact
head; and (O) that human merge authorization names this exact PR and head.

Missing, ambiguous or stale evidence at any point is a `SAFETY_BLOCKER`, never an assumption.

`WORKFLOW_CHANGE_INVALIDATES_ACCEPTANCE` — when the head workflow bytes or the workflow inventory
differ from the accepted base, the change is protected control-plane work: prior acceptance does not
carry over and a fresh independent semantic audit is required. When they are identical, evidence may
be reused after live re-proof. A candidate can never update its own external acceptance by editing
workflow, doctrine, validator and oracle together: those edits change what must be audited, never
whether it was audited. No repository-controlled digest is external merge authority; a blob identity
may be REPORTED as evidence, and reporting it authorizes nothing.

`CONFIG_RACE_HONESTY` — fresh premerge re-proof narrows, but does not claim to atomically eliminate,
an authorized administrator changing protection or workflow configuration between proof and merge.
This control plane states that limit rather than implying an atomicity it does not have.

### 17.2 TESTED_REVISION_BINDING_V1

Proving that a run is "tied to the head" proves nothing about what was compiled and tested. For a
`pull_request` event GitHub checks out a synthetic merge revision, so the run's reported head SHA and
the revision actually tested are DIFFERENT objects. The independent audit accepts the head TREE; CI
tests the MERGE tree; when the base has moved those differ, and acceptance would otherwise transfer to
content nobody audited.

The controller therefore proves this evidence bundle, live, at the exact head:

<!-- TESTED_REVISION_EVIDENCE_BEGIN -->
- event
- audited_pr_head
- audited_head_tree
- current_base
...
- required_contexts
<!-- TESTED_REVISION_EVIDENCE_END -->

Relations, all binding:

1. `run_reported_head` is the head GitHub attributes the run to, and it must equal `audited_pr_head`
   for every accepted event: a run attributed to any other revision is a run of that revision, and
   none of its conclusions are evidence for the audited one. It is never assumed to be
   `actual_checkout_revision`.
2. `pull_request`: the tested revision must be a two-parent merge revision whose first parent is
   `current_base` and whose second parent is `audited_pr_head`. A first-parent mismatch means the base
   moved; a second-parent mismatch means the head moved. Either invalidates the audit/run tuple.
3. `pull_request`: `tested_revision_tree` must equal `audited_head_tree`. A difference is a
   `SAFETY_BLOCKER` — CI tested content the independent audit never accepted.
4. `push`: the tested revision must equal the audited revision and has no synthetic merge relation.
5. Any other event is not merge-gate evidence without a separately designed and audited contract.
6. `checkout_ref_override` must be exactly `false`. An explicit checkout-ref override is NOT
   accepted merge-gate evidence in this control plane, and there is no branch that accepts one: the
   default merge-revision relation is the only checkout contract V2.1 implements. This is a stated
   LIMITATION, not a capability — an earlier revision described an accepting "prove it from run
   evidence" path that no code implemented, which is worse than having no path at all, because a
   reader could plan around it. Accepting overrides needs a separately designed and audited contract
   and a real implementation; until then an override is a `SAFETY_BLOCKER`. The repository validator
   does NOT parse workflow semantics.
7. `tests_job_conclusion` and every entry of `agent_os_gate_step_conclusions` must be `success`.
   `skipped`, `neutral` and `cancelled` are never acceptance.
8. A required status context does not identify a workflow file, path or revision, so the run's own
   source `workflow_path` must be proven. A sibling workflow publishing a green `tests` context is not
   a substitute for the expected run's provenance.
9. `pull_request`: `current_base` must differ from `audited_pr_head`, and `actual_checkout_revision`
   must be the synthetic merge revision itself — never the audited head or the base it merges.
10. `required_contexts` must include `tests`, the context of the one job whose conclusion the bundle
    carries. The bundle carries no conclusion for any other required context; proving those green
    stays the controller's live duty (section 17.1).

`RUN_HEAD_BINDING` — every field of the bundle is bound by at least one relation above, with one
deliberate exception: `workflow_run_id` is the provenance KEY. Its shape is checked; that every
run-derived field was read from that one run is the controller's live duty, and relation 1 is what
ties that run to the audited head. `current_base` binds only for `pull_request`, because a `push`
tested revision has no synthetic merge.

`tested_revision_failures()` in the validator makes these relations executable over a SUPPLIED evidence
bundle, so the rule is testable rather than prose. It judges evidence only: it contacts nothing,
establishes nothing about GitHub, and an empty result means the supplied bundle satisfies the relation
— never that the bundle is true. Establishing the bundle is the controller's duty.
...
registered above with a safe role, OR declared a historical non-applying surface below, OR absent.
There is no fourth state.

`HOST_DISCOVERY_CLOSED_WORLD` — the deterministic scan covers exactly the locations declared in
...
the first path segment of a declared location — must lie in a declared location or be listed in
`HOST_NON_DISCOVERY_PATHS`, so a registered or retired surface can never again sit in a host root
nobody scans. The scan claims nothing about host conventions that no registry of this control plane
names; those remain the independent audit's problem, not the scanner's.
...
`HISTORICAL_HOST_SURFACES` — a surface a host could still load, kept only as historical reference and
only in a role that cannot apply it. `NON_APPLYING` is the one role, and it is POSITIVE PROOF, never the
absence of a known unsafe spelling. Format: `- <path> :: NON_APPLYING`.

`NON_APPLYING_FRONT_MATTER_CONTRACT` — the role holds only where a non-application contract exists for
the surface's host location, and only the Cursor rule location has one; a historical surface anywhere
else fails, because a label alone never proves that a host will not load it. The Cursor contract is a
bounded SAFE SUBSET, deliberately not a YAML parser: the file opens with a `---` line and the next
`---` line closes the front matter; every line between them is exactly one plain `key: value` entry,
with no indentation, comment, quoting, anchor, tag, flow syntax, control or line-separator character;
no key repeats; the only keys are `description` and `alwaysApply`, so `globs` — which attaches a rule
by file pattern — or any other key fails; `alwaysApply` is present with exactly the bare lowercase value
`false`; and `description`, when present, is a plain scalar. Anything outside that subset loses the
role: the validator never decides whether an unfamiliar spelling would actually apply, it refuses to
certify it. The setup audit reports this same contract and holds no judgement of its own.

<!-- HISTORICAL_HOST_SURFACES_BEGIN -->
- .cursor/rules/prdv3-constitution.mdc :: NON_APPLYING
<!-- HISTORICAL_HOST_SURFACES_END -->
...
control plane with no oracle. The anchor closes that circle from outside the registry: the validator
holds that path as a LITERAL CONSTANT, independent of any registry entry, and requires the file to
exist. Removing the registry entry therefore does not remove the requirement. That is a
repository-provable fact and the validator claims nothing beyond it — in particular it does not
claim that its own execution in CI is guaranteed (section 17.1).
...
proof-paired: a MEANINGFUL value with `PROVEN`, or exactly `null` with `UNKNOWN`. A value without its
evidence field, a value contradicting its evidence field, a missing half of the pair, or a populated
but semantically empty value presented as proof are all invalid.
...
question ("is the key populated") while the contract asks an evidential one ("does this carry
...
vocabulary is `TEXT_EVIDENCE`, `OPERATIONAL_TEXT`, `TOKEN_REPO`, `TOKEN_BRANCH`,
`HASH_IDENTIFIER`, `NONNEGATIVE_INT`, `POSITIVE_INT`, `NORMALIZED_ENUM` and `STRUCTURED_LIST`. A
field with no class, an unknown class, or a class that contradicts the field's schema shape fails the
build in either direction — so a new proof field cannot be added without stating how it is proven.
`HASH_IDENTIFIER` accepts any letter case, because Git does.

`TEXT_EVIDENCE` and `OPERATIONAL_TEXT` are deliberately different strengths, because one predicate
was previously asked to do two jobs. `TEXT_EVIDENCE` is for FREE-FORM OBSERVATIONAL text stored
verbatim from elsewhere, in whatever script the source used; it holds when at least one code point
has Unicode MAJOR general category `L`, `N`, `P` or `S`. Its claim is exactly that and no more:
`U+3164 HANGUL FILLER` is category `Lo` and `U+2800 BRAILLE PATTERN BLANK` is category `So`, so both
satisfy it while conveying nothing to a reader. No general Unicode category test can decide
otherwise, and the classification comes from the `unicodedata` table compiled into the RUNNING
interpreter — no claim is made about code points assigned in a newer Unicode version than that
interpreter ships. `OPERATIONAL_TEXT` is for LOAD-BEARING values a human acts on or the control plane
compares — a next action, a gate name, an evidence key, a blocker id, a model identity. Those are
written in this project's operational language, so the grammar is the bounded positive one actually
used: at least one ASCII letter or digit. That is a LEXICAL claim, never a semantic one — it proves
the value is written in the operational character set, not that the action is correct, current or
achievable. Filler-only values fail it because they contain no operational token, and no list of
forbidden characters exists anywhere in this control plane.
...
`PROOF_TARGET_BINDINGS` — proof pairing proves a value is evidenced; it never proved what the value is
evidence OF. Each proof-paired field therefore declares its target: `SELF` for an identity, `NONE`
for a field with no revision or pull-request target, or the identity it observes. A `PROVEN` field
whose target is not `PROVEN` observes nothing identified and is rejected — a tree without its commit,
a pull-request state or thread count without its pull-request number, a CI state or a completed-gate
list without its head. Completed gates are bound twice: each evidence key must also name the proven
head or its tree, because a gate proven at another revision is invalidated evidence
(`VALIDATION_BUDGET`, section 9). A `PROVEN` `OPEN` or `DRAFT` pull request beside a `PROVEN` count of
zero open pull requests is rejected too, because the proven open pull request is itself a member of
the open set. The registry must equal the executable bindings and cover every proof-paired field, so
a new proof field cannot be added without stating what it observes. Format: `- <field> :: <TARGET>`.

<!-- PROOF_TARGET_BINDINGS_BEGIN -->
- branch :: SELF
- base_sha :: SELF
- base_tree :: base_sha
- head_sha :: SELF
...
### 20.1 Executable subordinate surfaces

`scripts/crypto_core/validate_agent_os_v2.py` and `tests/crypto_core/test_agent_os_v2_contract.py` are
EXECUTABLE surfaces, not doctrine. They carry exactly one `CONTROL_PLANE_ROLE: EXECUTABLE_SUBORDINATE`
...
`CANONICAL_NEGATIVE_BOUNDARY` — declared here once, reproduced verbatim by each executable:

<!-- EXECUTABLE_NEGATIVE_BOUNDARY_BEGIN -->
The repository validator proves repository-provable structure only. It does NOT prove that its own GitHub CI step executed, that GitHub will block a merge, or that a required status context identifies a particular workflow file or revision.
<!-- EXECUTABLE_NEGATIVE_BOUNDARY_END -->

The deterministic check over this is finite and says so: it proves each executable carries one role
marker, one authority reference and this exact sentence, and that five specific retired statements
are absent. Those five are a historical REMOVAL RECEIPT for claims deleted in this change, not a
forbidden-phrase vocabulary — nothing here detects a newly-worded equivalent claim, and a paraphrase
remains the INDEPENDENT SEMANTIC AUDIT's responsibility.
...
It additionally does not claim: that the repository can prove its own CI step is executed; that a
required status context identifies any particular workflow file, path or revision; that parsing a
workflow's YAML establishes GitHub's runtime execution semantics; that a repository-controlled digest
or self-check is external acceptance or merge authority; that premerge configuration re-proof is
atomic against an authorized administrator change; that a completed gate's evidence key reflects the
path set, command or environment actually run, when the manifest binds only the revision the key
names; that the host-discovery scan covers a host convention no registry of this control plane names;
that the ACTIVE projection understands prose, when it is decided by markers alone; that `NON_APPLYING`
proves more than that a host never applies the rule automatically, to every request or by file
pattern, when a host may still offer it on an agent request by its description or on an explicit
reference by name, which is why that description marks it historical;
or that a manifest satisfying the JSON Schema has
thereby satisfied the cross-field and meaningful-value contract, which only the semantic checker
enforces.
...
- 2026-09 — GPT-6 Astra became the canonical protected frontier lane and, for a short period, also
  ...
  valid historical evidence at their original dates; the ACTIVE disposition is section 3.5.
