...
### 4.3 Bounded implementation profile (Terra, Claude Sonnet 5)
...
- These lanes carry NO review duty. Ordinary review is section 4.3a; do not compile a review prompt
  against this profile.

### 4.3a Repo-native implementation and ordinary review profile (Codex GPT-5.6 Sol)

- Two duties only: bounded clear-spec implementation or repair, and ordinary fresh-context review.
  State which one the prompt is, explicitly.
- Review prompts name the exact read set, the exact contract under review, and the exact P1/P2/P3
  classification expected back. A review never becomes a repair without a separate task.
- Prefer this lane for ordinary review when another lane implemented the work, and prefer another
  lane when this one did — the reason is independence, not capacity.
- Never compile heavy implementation, capability-critical implementation or protected Class-C work
  against this profile; canonical routing prohibits all three for this lane.
...
### 4.5 Execution-environment profile (Work)

- Substantial multi-step execution, research and synthesis, under canonical section 7; never governance
  authority. Work draws on the SAME shared provider pool as Codex and the protected frontier lane, so
  `WORK_ENVIRONMENT_VALUE` must justify `SHARED_OPENAI_POOL_COST`: dispatch it for cloud browser or
  computer use, work across many files or apps, a persistent long workflow, cross-service evidence
  collection, large source synthesis, artifact production or multi-source current research — never for a
  simple GitHub read, a status question, a single-source question or a mechanical task.
...
the protected frontier lane named in canonical section 3.5; if that lane cannot run, the correct
