...
- Phase 25L adds: DERIBIT_BOOK_PARSE_SEQUENCE_PROOF.json fixture promoting change_id and prev_change_id
...
| `change_id` | claim_review | PROOF_READY_NOT_APPROVED | DERIBIT_BOOK_PARSE_SEQUENCE_PROOF.json + committed harness _sequence_id_from_data() + test_phase23b assert payload_sample['change_id'] | Harness mapping change_id->sequence_id is committed code with test coverage. Smoke run Phase 23L accepted 19 messages with 0 rejections confirming the mapping was exercised. No live integer values committed; human operator approval still required. |
| `prev_change_id` | claim_review | PROOF_READY_NOT_APPROVED | DERIBIT_BOOK_PARSE_SEQUENCE_PROOF.json + committed harness _prev_sequence_id_from_data() + test_phase23b test_sequence_gap_rejects using prev_change_id fixture | Harness mapping prev_change_id->prev_sequence_id is committed code with test coverage including sequence-gap detection that uses prev_change_id. No live integer values committed; human operator approval still required. |
...
## Phase 25L Promoted Claims (PROOF_READY_NOT_APPROVED)

### `change_id` — PROOF_READY_NOT_APPROVED

Evidence chain:
1. `src/crypto_core/data/deribit_public_ws_harness.py` `_sequence_id_from_data()` — committed function
   that reads `data.get("change_id")` as first-priority field, mapped to `DeribitPublicWsSmokeEvent.sequence_id`.
2. `_payload_sample()` — committed function that explicitly includes `"change_id"` in every event sample dict.
3. `tests/crypto_core/data/test_phase23b_deribit_public_ws_harness.py` — committed test
   `test_mocked_valid_message_produces_accepted_quarantine_summary` asserts
   `result.sample_events[0].payload_sample["change_id"] == 10` via fixture `change_id=10`.
4. `DERIBIT_PUBLIC_SMOKE_PROOF_RECORD.md` Phase 23L run_id=25671516104 — `accepted=true`,
   `message_count=19`, `rejection_reasons=[]` — confirms the harness processed 19 messages
   without fault using these same code paths.
5. `DERIBIT_BOOK_PARSE_SEQUENCE_PROOF.json` — this batch's committed fixture documenting
   the harness field-mapping architecture. `"phase25l_classification": "PROOF_READY_NOT_APPROVED"`.

Classification rationale: The harness field mapping from Deribit `change_id` field to
`sequence_id` is proven by committed source code + committed passing test. The smoke run
confirms the code path was exercised in production-equivalent conditions. Human operator
worksheet review remains required before APPROVED status.

### `prev_change_id` — PROOF_READY_NOT_APPROVED

Evidence chain:
1. `src/crypto_core/data/deribit_public_ws_harness.py` `_prev_sequence_id_from_data()` — committed function
   that reads `data.get("prev_change_id")` as first-priority field, mapped to `DeribitPublicWsSmokeEvent.prev_sequence_id`.
2. `_event_from_payload()` — uses `prev_sequence_id` in the continuity/gap-detection logic:
   `if prev_sequence_id != previous_sequence_id: reasons.append("deribit_ws:sequence_gap")`.
3. `tests/crypto_core/data/test_phase23b_deribit_public_ws_harness.py` — committed test
   `test_sequence_gap_rejects_when_sequence_fields_exist` uses fixture `prev_change_id=9`,
   `change_id=12` and asserts `"deribit_ws:sequence_gap" in result.rejection_reasons`,
   confirming `prev_change_id` is read and used in the harness logic.
4. `DERIBIT_PUBLIC_SMOKE_PROOF_RECORD.md` Phase 23L run_id=25671516104 — `accepted=true`,
   `message_count=19`, `rejection_reasons=[]` — confirms no sequence gaps were detected,
   meaning the `prev_change_id` check was exercised and passed.
5. `DERIBIT_BOOK_PARSE_SEQUENCE_PROOF.json` — this batch's committed fixture.
   `"phase25l_classification": "PROOF_READY_NOT_APPROVED"`.

Classification rationale: Same as `change_id` — committed code + test + smoke proof.
Human operator worksheet review still required before APPROVED status.

## Remaining WAIT_INSUFFICIENT Claims
...
1. Committed parse-sequence fixture recording actual observed `type`, `change_id`, `prev_change_id`
   values from a real Deribit book channel message sequence — required for:
   `first_message_snapshot`, `incremental_delta`, `continuity_condition`.
