...
$strictJson = "import sys; sys.path.insert(0, 'scripts/crypto_core'); import validate_agent_os_v2 as agent_os; "
...
foreach ($f in $vscodeJson) {
  ...
  $global:LASTEXITCODE = $null
  & $python -B -c ($strictJson + 'agent_os.load_strict_json_file(sys.argv[1])') $f > $null 2>&1
...
if (-not (Test-Path -LiteralPath '.vscode/mcp.json')) {
  ...
  $global:LASTEXITCODE = $null
  $count = & $python -B -c ($strictJson + "d = agent_os.load_strict_json_file('.vscode/mcp.json'); print(len(d.get('servers') or {}))") 2>$null
