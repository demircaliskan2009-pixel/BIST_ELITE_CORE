...
# FILESYSTEM ACCESS AUTHORITY (agent_os_v2.md section 15). The ONE place the control plane touches the filesystem:
# portable path validation, conservative absence, status, read and decoding, discovery listing, and one retained
# observation per path per run. It replaces a boundary that failed four ways in one family: a directory whose DirEntry
# classification failed was filed as a non-directory and hid its descendants from discovery; an embedded NUL became
# "missing" or a raw ValueError; an UNREADABLE bootstrap status was discarded and the path asked again; and a mirrored
# pathlib "does not exist" list turned a symlink loop or a device that was not ready into absence.
FILE_PRESENT = "PRESENT"
FILE_MISSING = "MISSING"
FILE_NOT_REGULAR = "NOT_A_FILE"
FILE_UNREADABLE = "UNREADABLE"
FILE_INVALID_PATH = "INVALID_PATH"
...
_ABSENCE_WINERRORS = {errno.ENOENT: frozenset({2, 3}), errno.ENOTDIR: frozenset({267})}
# A Windows reparse point whose tag carries the name-surrogate bit is a symbolic link or a junction.
_NAME_SURROGATE_REPARSE = 0x20000000
_WINDOWS_DEVICE_NAMES = frozenset(
    {"CON", "PRN", "AUX", "NUL"} | {f"COM{index}" for index in range(1, 10)} | {f"LPT{index}" for index in range(1, 10)}
)
_PATH_FORBIDDEN_CHARACTERS = frozenset('<>:"|?*\\')
_PATTERN_FORBIDDEN_CHARACTERS = frozenset('<>:"|\\')


class FileObservation(NamedTuple):
    """One observation of one path.

    ``kind`` names what a present node is (file, directory, symbolic link or junction, special file); ``text`` is set
    only once a PRESENT file has been read and decoded; ``reason`` explains UNREADABLE and INVALID_PATH.
    """
...
def repository_path_failure(value: object, *, pattern: bool = False) -> str | None:
    """Why ``value`` is not a portable repository-relative path (or discovery pattern), or None.

    The grammar is platform-independent on purpose. A registry entry one platform cannot name must fail on every
    platform, instead of reading as "missing" where the OS says so - Windows reports `CON`, `a:b` and `x.` as not
    found - and raising where it does not. A pattern additionally allows `*` and `?`, and `**` as a whole segment.
    """
    ...
    present = sorted({character for character in value if character in forbidden})
    if present:
        return "forbidden character(s) " + " ".join(repr(character) for character in present)
    if value.startswith("/"):
    ...
    for segment in value.split("/"):
        ...
        if segment.endswith((".", " ")):
            return f"segment {segment!r} ends with '.' or a space"
        if segment.split(".", 1)[0].upper() in _WINDOWS_DEVICE_NAMES:
            return f"segment {segment!r} is a reserved device name"
    return None


def operator_path_failure(value: object) -> str | None:
...
def _positively_absent(exc: OSError) -> bool:
    if not isinstance(exc, (FileNotFoundError, NotADirectoryError)):
        return False
    allowed = _ABSENCE_WINERRORS.get(exc.errno)
    if allowed is None:
        return False
    winerror = getattr(exc, "winerror", None)
    return winerror is None or winerror in allowed
...
def read_observed_text(path: Path, observed: FileObservation) -> FileObservation:
    """Read and decode a file whose PRESENT status is already retained, without observing its status again.

    A read that fails after that status - the file vanished, became unreadable or became a directory - is UNREADABLE,
    never MISSING: the retained observation already said the file was there, so the race fails closed. A link swapped
    in after the status was taken is followed by the read; defending that race is a non-claim (section 21).
    """
    if observed.status != FILE_PRESENT or observed.text is not None:
        return observed
    try:
...
class FileLedger:
    ...
    def text(self, rel: str) -> FileObservation:
...
def observe_root(raw: str | None) -> tuple[Path | None, str | None]:
    """The repository root to validate, or the one structured reason it cannot be used - never an exception."""
    if raw is None:
        return repo_root_from_here(), None
    problem = operator_path_failure(raw)
    if problem is not None:
        return None, f"--root {raw!r}: {INVALID_PATH}: {problem}"
    try:
...
def discover_files(root: Path, pattern: str, ledger: FileLedger | None = None) -> tuple[list[str], list[str]]:
    ...
    if problem is not None:
        return [], [f"host auto-discovery pattern {pattern!r}: {INVALID_PATH}: {problem}"]
    ...
    while stack:
        ...
        for name in sorted(entries):
            ...
            if kind == KIND_FILE:
                ...
                failures.append(
                    f"host auto-discovery entry {rel} (matched {pattern}) is not a regular file, so it cannot be "
                    f"proven registered or absent"
                )
...
def read_projection(root: Path, rel: str, ledger: FileLedger | None = None) -> ActiveProjection | None:
    """Read and project one authority surface through the file ledger. None when the file is positively absent.

    A doctrine surface is projected whole. An executable subordinate is projected from its module docstring,
    padded so every line keeps its original number. A surface whose authority text cannot be obtained - an invalid
    path, a path that is not a regular file, a status, read or decode failure, or an executable with no docstring -
    is an EMPTY projection carrying that reason, so it fails structurally and no reader can find authority in it.
    """
    observed = (ledger if ledger is not None else FileLedger(root)).text(rel)
    if observed.status == FILE_MISSING:
        return None
    if observed.status == FILE_NOT_REGULAR:
        return ActiveProjection((f"{rel}: not a regular file ({observed.kind}), so it carries no authority text",), ())
...
def surface_view(root: Path, ctx: dict[str, object], rel: str) -> ActiveProjection | None:
    ...
    if rel not in views:
        views[rel] = read_projection(root, rel, ctx["ledger"])  # type: ignore[arg-type]
    return views[rel]
...
def _check_registries(root: Path, failures: list[str]) -> dict[str, object] | None:
    ...
    observed = ledger.text(CANONICAL)
    if observed.status == FILE_MISSING:
        failures.append(f"canonical authority missing: {CANONICAL}")
        return None
    if observed.status == FILE_NOT_REGULAR:
        failures.append(f"canonical authority is not a regular file: {CANONICAL} ({observed.kind})")
        return None
    if observed.text is None:
    ...
    # Every registry path and discovery pattern passes the portable path grammar before anything observes it, so an
    # entry no platform should be asked about - an embedded NUL, a '..' escape, a device name - is a structured
    # rejection instead of "missing" on one platform and a raw ValueError on another.
    path_registries = (
        (BLOCK_ACTIVE_SURFACES, surface_paths),
        (BLOCK_REQUIRED_ARTIFACTS, artifacts),
        (BLOCK_DURABLE_SURFACES, durable),
    ...
    for label, values in path_registries:
        for value in values:
            problem = repository_path_failure(value)
            if problem is not None:
                failures.append(f"{CANONICAL}: {label} entry {value!r}: {INVALID_PATH}: {problem}")
    for value in host_globs:
...
def required_file_failures(ledger: FileLedger, rel: str, label: str) -> list[str]:
    """A file required only to EXIST, judged by its ONE retained status: its content is never read."""
    ...
    if observed.status == FILE_NOT_REGULAR:
        return [f"{label} is not a regular file: {rel} ({observed.kind})"]
    return [f"{label} status cannot be proven: {rel}: {observed.reason}"]
...
def _check_existence(root: Path, ctx: dict[str, object], failures: list[str]) -> None:
    ...
    for path in ctx["artifacts"]:  # type: ignore[union-attr]
        failures.extend(required_file_failures(ledger, path, "required control-plane artifact"))
    # A retired path must be PROVEN absent: an invalid path or a status that cannot be read is never absence.
    for path in ctx["retired"]:  # type: ignore[union-attr]
        observed = ledger.status(path)
        if observed.status in (FILE_PRESENT, FILE_NOT_REGULAR):
    ...
    # Anchored on a literal constant, NOT on the mutable required-artifact registry, so removing the registry entry
    # does not remove the requirement. Its ONE retained observation decides; nothing asks the filesystem again.
    oracle = ledger.status(BOOTSTRAP_ORACLE_PATH)
    if oracle.status == FILE_MISSING:
        failures.append(
        ...
        failures.extend(required_file_failures(ledger, BOOTSTRAP_ORACLE_PATH, "independent contract oracle"))
...
def load_strict_json_file(path: Path | str) -> object:
    """Read an operator JSON file through the filesystem access authority and parse it through `load_strict_json`.

    The path is operator evidence, so it may be a link and is observed following it; it still passes the operator path
    check first, so an embedded NUL is INVALID_PATH, never "missing" and never a raw ValueError.
    """
    raw = os.fspath(path)
    problem = operator_path_failure(raw)
    if problem is not None:
    ...
    if observed.status == FILE_NOT_REGULAR:
        raise StrictJsonError(f"cannot be read as UTF-8 text (not a regular file: {observed.kind})")
    if observed.text is None:
        raise StrictJsonError(f"cannot be read as UTF-8 text ({observed.reason})")
    return load_strict_json(observed.text)


def committed_json(ledger: FileLedger, rel: str, failures: list[str]) -> tuple[str, object] | None:
    ...
    """
    observed = ledger.text(rel)
    if observed.status == FILE_MISSING:
        failures.append(f"{rel}: missing")
        return None
    if observed.status == FILE_NOT_REGULAR:
        failures.append(f"{rel}: {STRICT_JSON_REJECTED}: not a regular file ({observed.kind})")
        return None
    if observed.text is None:
...
def _check_manifest_grammar(root: Path, ctx: dict[str, object], failures: list[str]) -> None:
    ...
    loaded = committed_json(ctx["ledger"], schema_path, failures)  # type: ignore[arg-type]
    if loaded is None:
        return
    try:
...
def _check_continuity_example(root: Path, ctx: dict[str, object], failures: list[str]) -> None:
    ...
    loaded = committed_json(ctx["ledger"], example_path, failures)  # type: ignore[arg-type]
    if loaded is None:
        return
    raw, example = loaded
...
def collect_failures(root: Path) -> list[str]:
    ...
    problem = operator_path_failure(os.fspath(root))
    if problem is not None:
        return [f"root {os.fspath(root)!r}: {INVALID_PATH}: {problem}"]
    failures: list[str] = []
    ...
    _check_continuity_example(root, ctx, failures)
    return failures
...
def repo_root_from_here() -> Path:
    """The repository containing this script, derived from its own absolute path without resolving anything."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    root, root_problem = observe_root(args.root)
    ...
    if args.manifest is not None:
        manifest_failures = ([root_problem] if root_problem is not None else []) + check_manifest_file(
            root if root is not None else repo_root_from_here(), Path(args.manifest)
        )
        if args.json:
            _emit(
                json.dumps(
    ...
    failures = [root_problem] if root is None else collect_failures(root)

    if args.json:
        shown_root = str(root) if root is not None else args.root
        _emit(json.dumps({"root": shown_root, "ok": not failures, "failures": failures}, indent=2, sort_keys=True))
    elif failures:
