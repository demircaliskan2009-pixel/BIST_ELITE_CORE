...
def _check_registries(root: Path, failures: list[str]) -> dict[str, object] | None:
    ...
    historical_host = parse_surface_registry(canonical_text, BLOCK_HISTORICAL_HOST)
    host_non_discovery = parse_registry(canonical_text, BLOCK_HOST_NON_DISCOVERY)
...
ALWAYS_APPLY_TRUE_RE = re.compile(r"^\s*alwaysApply\s*:\s*true\s*$", re.IGNORECASE | re.MULTILINE)
...
def front_matter(text: str) -> tuple[bool, str | None]:
    """(opened, body). A file that opens front matter and never closes it has no provable body."""
    lines = text.splitlines()
...
def _check_host_discovery(root: Path, ctx: dict[str, object], failures: list[str]) -> None:
    ...
    a host root nobody scans. The scan claims nothing about host conventions no registry names.
    """
    ...
    for path, role in historical:
        ...
        if text is None or role != "NON_APPLYING":
            continue
        opened, body = front_matter(text)
        if opened and body is None:
            failures.append(f"{path}: front matter never closes, so NON_APPLYING cannot be proven")
        elif body is not None and ALWAYS_APPLY_TRUE_RE.search(body):
            failures.append(
                f"{path}: a NON_APPLYING historical host surface declares alwaysApply: true, so a host "
                f"would apply it to every request"
            )
...
def _check_manifest_grammar(root: Path, ctx: dict[str, object], failures: list[str]) -> None:
    ...
    bindings = parse_surface_registry(ctx["canonical_text"], BLOCK_PROOF_TARGETS)  # type: ignore[arg-type]
    if bindings is None:
