...
# Exact content proves WHAT would run; it proves nothing about WHETHER it runs, or whether its
# failure can fail the job. That is the execution CONTEXT, and it is carried by `if`,
# `continue-on-error` and `shell`. Their values may be literals, GitHub expressions, or values
# with a trailing comment, so comparing values would mean shipping an expression evaluator and
# a YAML engine. The contract is therefore narrow and structural: on the required job and on its
# two gate steps these keys must be ABSENT, and PRESENCE alone fails, whatever the value says.
REQUIRED_CI_JOB = "tests"
FORBIDDEN_CI_JOB_KEYS = ("if", "continue-on-error")
FORBIDDEN_CI_GATE_STEP_KEYS = ("if", "continue-on-error", "shell")

# A YAML mapping key, with its indent. A standalone comment can never match, so `# if: false`
# declares nothing, while `if: false # disabled` really does declare `if`.
YAML_KEY_RE = re.compile(r"^(\s*)([A-Za-z0-9_.-]+):\s*(.*)$")

# Job-level keys sit at this indent inside a body returned by _workflow_job_bodies.
JOB_KEY_INDENT = 4
...
def _yaml_key(line: str) -> str | None:
    """The YAML key a line declares, or None. A standalone comment declares nothing."""
    match = YAML_KEY_RE.match(line)
    return None if match is None else match.group(2)


def _child_lines(lines: list[str], indent: int, key: str) -> list[str] | None:
    """Lines nested strictly inside `key` declared at `indent`, or None when it is absent."""
    for i, line in enumerate(lines):
        match = YAML_KEY_RE.match(line)
        if match is None or len(match.group(1)) != indent or match.group(2) != key:
            continue
        body: list[str] = []
...
def _defaults_declare_shell(lines: list[str], indent: int) -> bool:
    """True when a `defaults:` block at `indent` sets a run shell for the steps below it.

    ANY `shell` key inside that block counts. Demanding an exact defaults -> run -> shell indent
    chain would let a differently indented but equally effective override through, and a gate
    whose shell can be replaced by `echo {0}` keeps its exact command and runs nothing.
    """
    body = _child_lines(lines, indent, "defaults")
    if body is None:
        return False
    return any(_yaml_key(line) == "shell" for line in body)


def _job_has_key(job_lines: list[str], key: str) -> bool:
    """True when the job declares `key` at job level, whatever its value is."""
    for line in job_lines:
        match = YAML_KEY_RE.match(line)
        if match and len(match.group(1)) == JOB_KEY_INDENT and match.group(2) == key:
            return True
    return False


def _step_has_key(step_lines: list[str], key: str) -> bool:
    """True when a step declares `key`, whatever its value - `if: false # disabled` included."""
    inline, block = _step_key(step_lines, key)
...
def _find_exact_step(steps: list[list[str]], command: str) -> str | None:
    ...
    for step in steps:
        ...
        if commands != [command]:
            continue
        for key in FORBIDDEN_CI_GATE_STEP_KEYS:
            if _step_has_key(step, key):
                return (
                    f"the dedicated step running {command!r} declares '{key}'; a required gate "
                    f"step may not carry that key in any form"
                )
        return None
    if saw_mention:
        return (
...
def _check_ci_wiring(root: Path, failures: list[str]) -> None:
    ...
    if _defaults_declare_shell(lines, 0):
        failures.append(
            f"{ci_path}: a workflow-level 'defaults.run.shell' can replace the shell that runs "
            f"the required gates; it must be absent"
        )

    job_bodies = _workflow_job_bodies(lines)
    if REQUIRED_CI_JOB not in job_bodies:
        failures.append(f"{ci_path}: required job {REQUIRED_CI_JOB!r} not found in the jobs mapping")
        return

    body = job_bodies[REQUIRED_CI_JOB]

    for key in FORBIDDEN_CI_JOB_KEYS:
        if _job_has_key(body, key):
            failures.append(
                f"{ci_path}: the required {REQUIRED_CI_JOB!r} job declares '{key}'; the required "
                f"job may not carry that key in any form"
            )

    if _defaults_declare_shell(body, JOB_KEY_INDENT):
        failures.append(
            f"{ci_path}: the required {REQUIRED_CI_JOB!r} job declares 'defaults.run.shell'; it "
            f"must be absent so the gate steps run in the default shell"
        )

    steps = _job_steps(body)
    if not steps:
        failures.append(f"{ci_path}: the {REQUIRED_CI_JOB!r} job declares no steps")
        return
