...
#
# TEXT_EVIDENCE_LNPS_V1. Evidence text is defined by what it CONTAINS, not by how it renders.
# A value carries a token payload when at least one code point has a Unicode general category
# whose MAJOR class is L, N, P or S; a string drawn entirely from M, Z and C carries none.
# This is the Unicode Standard's own semantic classification, so a code point assigned in a
# future Unicode version is covered on the day it is assigned and there is no list to grow.
# It proves ONLY that a payload is present - never that the value is true, current or
# well-sourced, which remains the evidence source's and the independent audit's concern.
...
MODEL_RUNTIME_GRAMMAR = _object(
    ...
    description=(
        ...
        "runtime metadata says so with model_evidence_source UNKNOWN and null actual values - absence "
        ...
        "an unexpected model or an observed fallback can always be recorded."
    ),
)
    ...
    if not isinstance(runtime, dict):
        ...
        if source not in MODEL_EVIDENCE_CLASSES:
            ...
            if text_evidence_failures(actual):
                failures.append(f"{label}: RUNTIME_TELEMETRY claims runtime proof but model_actual carries no payload")
        elif source == "USER_ATTESTED_UI_SELECTION":
            if text_evidence_failures(actual) and text_evidence_failures(host_raw):
                failures.append(
...
def tested_revision_failures(evidence: object) -> list[str]:
    ...
    if event not in ACCEPTED_TESTED_EVENTS:
        return [f"event {event!r} is not accepted as merge-gate evidence without a separate audited design"]

    checkout = evidence["actual_checkout_revision"]
    if checkout is None or (isinstance(checkout, str) and not checkout.strip()):
        return ["SAFETY_BLOCKER: the actual tested revision could not be proven"]

    # The run's reported head is metadata. It is never assumed to be what was checked out.
    if evidence["checkout_ref_override"]:
        return [
            "SAFETY_BLOCKER: the workflow overrides the checkout ref, so the default merge-revision "
            "relation does not apply and the tested revision must be proven from run evidence"
        ]

    parents = evidence["tested_revision_parents"]
    if not isinstance(parents, (list, tuple)):
        return ["tested_revision_parents must be a list"]
...
def _check_manifest_grammar(root: Path, ctx: dict[str, object], failures: list[str]) -> None:
    ...
    try:
        ...
        return
    if committed != emit_manifest_schema():
        failures.append(
