...
- Independence credit is decided ONLY by canonical Agent OS routing and independence rules
  (sections 2.4, 3.1 and 3.4); this adapter does not define it. The host consequence is the narrow
  one: a Claude review of Claude-authored work is same-family and therefore
  `SELF_AUDIT_ONLY_NOT_INDEPENDENT`, and no Claude lane satisfies protected Class C. Protected
  Class-C work always gets a fresh-context audit from the protected frontier lane, and an
  unavailable frontier lane is `CLASS_C_LANE_REQUIRED_BUT_UNAVAILABLE` — never a silent downgrade.
