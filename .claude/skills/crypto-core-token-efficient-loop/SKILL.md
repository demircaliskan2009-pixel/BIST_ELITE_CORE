...
active content `MINIMAL_OPERATIONAL_CONTROL_PLANE_KERNEL_V1` as amended, most recently by
`CODEX_QUOTA_RESILIENCE_V2`). This checklist restates no authority. Token
...
5. Close the whole semantic boundary as the largest safe semantic closure (§24.8) inside the allowed files,
   preserving paper-only, fail-closed and digest-boundary rules, with an exhaustive self-audit. A repair is the
   single consolidated repair: the complete controller-adjudicated P1/P2 set of the `REPAIR_ENTRY_MODE` its prompt
   names (§24.8), by root cause, in one change.
...
8. End with an `AGENT_OS_HANDOFF_V1` handoff: runtime proof, setup fields (`SETUP_REQUESTED`/`SETUP_ACTUAL`/
   `SETUP_FILES_READ`/`SETUP_GAPS`), exact scope, validation, PR/CI/thread evidence, the ACTUAL meaningful-prompt
   and challenge counts (never a fixed number), `SELF_AUDIT_ONLY_NOT_INDEPENDENT`, and exactly one next safe
   action.

## Boundaries

- Council (§24.3): ChatGPT controller (`CONTROLLER_READONLY_FIRST_POLICY`), the default non-protected acceptance
  auditor; Claude Opus 5.5 (`claude-opus-5-5`) for deep semantic implementation and repair, including the
  repo-native work of that same task, and the optional, default-off read-only challenge (evidence only); GPT-6
  Astra as the SOLE protected acceptance auditor (`ASTRA_UNIFIED_INDEPENDENT_AUDIT_V1`), only on a trigger of
  `PROTECTED_TRIGGER_MATRIX_V2` and never reassigned; Codex GPT-5.6 Sol as `SOL_RESERVE_ONLY` (§24.3, §24.4);
  ChatGPT Work; Deep Research. Claude Sonnet 5, Codex Terra and
...
- Budget (§24.8 `HARD_FIVE_EXECUTION_BUDGET`): one budget, hard maximum 5, no sixth; implementation, acceptance
  audit, repair, re-audit and any challenge all count; target the fewest (2 clean, 4 repaired); controller
  governance counts zero. One exhaustive acceptance audit, at most one consolidated repair, one whole-contract
  re-audit. Any genuine material P1/P2 after that → `FIXED_POINT_STOP`; P3 never blocks; no mutation of a
  rejected or frozen candidate, including a protected head frozen while GPT-6 Astra is unavailable.
- No Claude session accepts a candidate or satisfies an acceptance or protected audit. A challenge run is
  READ_ONLY, fresh-context and runtime-proven, returns every material P1/P2 it finds with evidence, and is
  evidence only (on Opus 5.5 work also `SELF_AUDIT_OR_SAME_MODEL_CHALLENGE_NOT_INDEPENDENT`).
- `USER_MANUAL_WORK_MINIMIZATION` (§24.10): never ask the human for a command or fact the session can produce.
