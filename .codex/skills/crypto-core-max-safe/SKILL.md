...
- **Bounded lane (Terra).** Exact-file deterministic implementation, tests and docs, small
  same-branch repairs. Bounded implementation only — Terra carries no review duty.
- **Repo-native lane (Codex GPT-5.6 Sol).** Exactly two duties: bounded clear-spec implementation or
  repair, and ordinary fresh-context review. Prefer Sol for the review when another lane implemented
  the work, and prefer another lane when Sol implemented it — a same-model review is
  `SELF_AUDIT_ONLY_NOT_INDEPENDENT` here too. Never a protected Class-C substitution, and never heavy
  or capability-critical implementation.
- **Protected frontier lane.** Class-C protected design and audit run on a controller-prepared narrow
  ...
  The canonical protected frontier lane is named in canonical section 3.5; if it cannot run, return
