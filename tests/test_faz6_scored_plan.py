...
def test_scored_decisions():
    ...
    bands = [
        PriceBand(price_min=0.01, price_max=1000000.0, tick=0.01, up_limit_pct=20.0, down_limit_pct=20.0)
    ]
    ...
    kap_events = {
        ...
        "B": [],  # Nötr
        "C": [
            {"headline": "Soruşturma başlatıldı", "title": "İptal", "raw": {"headline": "Soruşturma"}}
        ]
    }
    ...
    assert results_by_symbol["A"]["decision_raw"] == "BUY", f"A için decision_raw: {results_by_symbol['A']['decision_raw']}, score: {results_by_symbol['A']['score']}"
    ...
    assert results_by_symbol["C"]["decision_raw"] == "PASS", f"C için decision_raw: {results_by_symbol['C']['decision_raw']}, score: {results_by_symbol['C']['score']}"
