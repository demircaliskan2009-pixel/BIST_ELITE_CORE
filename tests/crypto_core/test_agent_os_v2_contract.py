...
def _run_validator_only(extra: list[str], seed_stale_success: bool) -> subprocess.CompletedProcess:
    ...
    args = " ".join("'{}'".format(item) for item in extra)
...
def test_setup_audit_fails_closed_when_the_validator_cannot_launch() -> None:
    ...
    assert "VALIDATOR_EXIT=UNKNOWN" in done.stdout, done.stdout
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            ("- GPT-6 Astra :: gpt-6-astra :: READ_ONLY", "- GPT-6 Astra :: READ_ONLY"),
        ),
        (
            "a lane trusted with an unknown authority",
            ("- GPT-5.6 Terra :: - :: BOUNDED_MUTATION", "- GPT-5.6 Terra :: - :: OMNIPOTENT"),
        ),
    ],
    ids=[
...
ORACLE_SOL_AUTHORITIES = {"BOUNDED_MUTATION", "READ_ONLY"}


def test_the_sol_lane_is_declared_with_exactly_its_two_capabilities() -> None:
    rows, problems = validator.parse_columns(
        (REPO_ROOT / CANONICAL).read_text(encoding="utf-8-sig"), "LANE_CAPABILITY", 3
    )
    assert rows and not problems, problems
    declared = {lane: (model, {a.strip() for a in auth.split(",")}) for lane, model, auth in rows}
    assert ORACLE_SOL_LANE in declared, "the prose gives Sol duties; the lane must exist"
    model_id, authorities = declared[ORACLE_SOL_LANE]
    assert model_id == ORACLE_SOL_MODEL_ID
    assert authorities == ORACLE_SOL_AUTHORITIES, "Sol is bounded + read-only, never heavy or critical"
...
def test_no_active_surface_assigns_review_to_a_lane_that_cannot_review() -> None:
    ...
    cannot_review = {
        lane.replace("Codex ", "")
        for lane, _model, auth in rows
        if "READ_ONLY" not in {a.strip() for a in auth.split(",")}
    }
