...
_REAL_ANCHOR_STEP = (
    "      - name: Agent OS contract oracle anchor\n"
    "        run: test -f tests/crypto_core/test_agent_os_v2_contract.py\n\n"
)

_GATE_STEP_CONTEXT_ATTACKS = [
    ("CI-A1 step if false", "        if: false\n"),
    ("CI-A2 step if false behind a trailing comment", "        if: false # disabled\n"),
    ("CI-A3 step if constant-false expression", "        if: ${{ 1 == 0 }}\n"),
    ("CI-A7 step continue-on-error", "        continue-on-error: true\n"),
    ("CI-A8 step continue-on-error behind a trailing comment", "        continue-on-error: true # masked\n"),
    ("CI-A9 step continue-on-error expression", "        continue-on-error: ${{ true }}\n"),
    ("CI-A12 step custom shell", "        shell: echo {0}\n"),
]


@pytest.mark.parametrize(
    ("label", "injected"), _GATE_STEP_CONTEXT_ATTACKS, ids=[case[0] for case in _GATE_STEP_CONTEXT_ATTACKS]
)
def test_validator_gate_step_rejects_every_execution_context_key(sandbox: Path, label: str, injected: str) -> None:
    """The command stays exact; only its context changes. Presence of the key is the failure."""
    patch(
        sandbox,
        ".github/workflows/ci.yml",
        _REAL_CI_STEP,
...
def test_oracle_anchor_step_rejects_every_execution_context_key(sandbox: Path, label: str, injected: str) -> None:
    """CI-A17: both required gates carry the same protection, not just the validator one."""
    patch(
        sandbox,
        ".github/workflows/ci.yml",
        _REAL_ANCHOR_STEP,
...
_JOB_CONTEXT_ATTACKS = [
    ("CI-A4 job if false", "    if: false\n", "declares 'if'"),
    ("CI-A5 job if false behind a trailing comment", "    if: false # disabled\n", "declares 'if'"),
    ("CI-A6 job if constant-false expression", "    if: ${{ 1 == 0 }}\n", "declares 'if'"),
    ("CI-A10 job continue-on-error", "    continue-on-error: true\n", "declares 'continue-on-error'"),
    ("CI-A11 job continue-on-error expression", "    continue-on-error: ${{ true }}\n", "declares 'continue-on-error'"),
]


@pytest.mark.parametrize(
    ("label", "injected", "needle"), _JOB_CONTEXT_ATTACKS, ids=[case[0] for case in _JOB_CONTEXT_ATTACKS]
)
def test_required_job_rejects_every_execution_context_key(
    sandbox: Path, label: str, injected: str, needle: str
) -> None:
    """A job-level key defeats every step inside it however exact each step looks."""
    text = read(sandbox, ".github/workflows/ci.yml")
    write(
        sandbox,
        ".github/workflows/ci.yml",
...
def test_required_job_default_run_shell_is_rejected(sandbox: Path) -> None:
    """CI-A13: a job-level default shell replaces the shell every gate step runs in."""
    text = read(sandbox, ".github/workflows/ci.yml")
    write(
        sandbox,
        ".github/workflows/ci.yml",
    ...
    assert_rejects(sandbox, "defaults.run.shell")


def test_workflow_level_default_run_shell_is_rejected(sandbox: Path) -> None:
    """CI-A14: the same override one level up reaches the gates just as effectively."""
    text = read(sandbox, ".github/workflows/ci.yml")
    write(
        sandbox,
        ".github/workflows/ci.yml",
    ...
    assert_rejects(sandbox, "workflow-level 'defaults.run.shell'")


def test_committed_workflow_satisfies_the_execution_context_contract(sandbox: Path) -> None:
    """CI-A15: the control. The real workflow needs no override and declares none."""
    assert failures(sandbox) == []


def test_standalone_yaml_comments_do_not_create_execution_context_keys(sandbox: Path) -> None:
    """CI-A16: a commented-out key is a comment, not a key - fail-closed must not mean false-positive."""
    text = read(sandbox, ".github/workflows/ci.yml")
