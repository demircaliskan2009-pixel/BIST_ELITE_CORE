...
evidence admission record (with its end-to-end episode provenance) into the EXISTING ``EvidenceJournal``.

Covers the happy path, digest/provenance binding (no fake cross-contract equality), cross-artifact identity,
status/safety/non-overclaim, raw/canonical adversariality, forbidden-surface exclusion (alias-resistant),
journal append/ledger semantics (prior-head anchor, duplicate replay, non-empty journal, serialization
round-trip), and non-overclaim."""
...
_CORR = "corr-ep"  # runtime correlation shared by the episode chain (episode runner + realized event)
_REQ_CORR = "corr-req"
_RUN = "run-ep"
_EPISODE_ID = "e2e-1"
_UNSET = object()
...
# --------------------------------------------------------------------------------------------------
# Admission-record chain (manifest -> ADMITTED PaperEvidenceAdmissionRecord)
# --------------------------------------------------------------------------------------------------
...
def _mr_bundle(event_id: str, *, market_symbol: str = _SYMBOL):
...
def _mr_entry(idx: str = "1", *, market_symbol: str = _SYMBOL):
    prov = _mr_prov(idx, market_symbol=market_symbol)
    ents = (_mr_bundle(f"e{idx}", market_symbol=market_symbol),)
...
def _ready_manifest(*, market_symbol: str = _SYMBOL):
    agg = build_paper_session_realized_pnl_aggregate(
        [_mr_entry("1", market_symbol=market_symbol)], aggregate_id="agg-1", correlation_id="corr-agg"
    )
...
def _admitted_record(*, correlation_id: str = _CORR, market_symbol: str = _SYMBOL, metadata=None):
    manifest = _ready_manifest(market_symbol=market_symbol)
...
# --------------------------------------------------------------------------------------------------
# Episode chain (-> READY PaperEndToEndEpisode)
# --------------------------------------------------------------------------------------------------
...
def _e_realized(order_intent, prior):
...
def _episode_chain():
    ...
    bridge = _e_bridge()
    prior = _e_prior_long()
    return bridge, order_intent, _e_run(order_intent, prior), _e_realized(order_intent, prior)


def _build_episode(**overrides):
    bridge, order_intent, episode_run, realized = _episode_chain()
    kwargs = {
...
def _append(
    ...
    *,
    ...
    record = record if record is not None else _admitted_record()
    episode = episode if episode is not None else _ready_episode()
...
def test_valid_inputs_ready_and_appends_one_entry() -> None:
    ...
    journal = EvidenceJournal()
    bridge = _append(journal, record=record, episode=episode)
    ...
    assert bridge.market_symbol == _SYMBOL
    assert bridge.episode_id == _EPISODE_ID
    assert bridge.run_id == _RUN
    assert bridge.correlation_id == _CORR
...
def test_all_consumed_digests_are_bound() -> None:
    ...
    assert bridge.expected_admission_digest == record.admission_digest
    assert bridge.manifest_digest == record.manifest_digest
    assert bridge.aggregate_digest == record.aggregate_digest
    assert bridge.aggregate_id == record.aggregate_id
    assert bridge.episode_digest == episode.episode_digest
    assert bridge.expected_episode_digest == episode.episode_digest
    assert bridge.realized_pnl_event_digest == episode.realized_pnl_event_digest
    assert bridge.episode_run_digest == episode.episode_run_digest
    assert bridge.order_intent_digest == episode.order_intent_digest


def test_append_binds_journal_seam_digests() -> None:
    journal = EvidenceJournal()
    bridge = _append(journal)
    appended = journal.snapshot()[0]
    assert bridge.prior_journal_head_digest is None
    assert bridge.prior_journal_entry_count == 0
    assert bridge.appended_entry_seq == 0
    assert bridge.appended_entry_digest == appended.entry_digest
    assert bridge.appended_payload_digest == appended.payload_digest
    assert bridge.resulting_journal_head_digest == journal.head_digest
    assert bridge.resulting_journal_entry_count == 1


def test_ledger_digest_deterministic_and_recomputes() -> None:
    a = _append(EvidenceJournal())
    b = _append(EvidenceJournal())
...
def test_changed_bound_field_changes_ledger_digest() -> None:
    base = _append(EvidenceJournal())
    other = _append(EvidenceJournal(), metadata={"note": "second"})
...
def test_wrong_expected_admission_digest_rejects() -> None:
    journal = EvidenceJournal()
    bridge = _append(journal, expected_admission_digest="a" * 64)
...
def test_wrong_expected_episode_digest_rejects() -> None:
    journal = EvidenceJournal()
    bridge = _append(journal, expected_episode_digest="a" * 64)
    assert bridge.status is PaperAdmissionLedgerBridgeStatus.REJECTED
...
def test_forged_admission_self_digest_rejects() -> None:
    record = replace(_admitted_record(), admission_digest="0" * 64)
    journal = EvidenceJournal()
    bridge = _append(journal, record=record, expected_admission_digest="0" * 64)
    assert bridge.status is PaperAdmissionLedgerBridgeStatus.REJECTED
    assert any("admission_digest_mismatch" in r for r in bridge.rejection_reasons)
    assert journal.entry_count == 0


def test_forged_episode_self_digest_rejects() -> None:
    episode = replace(_ready_episode(), episode_digest="0" * 64)
    journal = EvidenceJournal()
    bridge = _append(journal, episode=episode, expected_episode_digest="0" * 64)
    assert bridge.status is PaperAdmissionLedgerBridgeStatus.REJECTED
    assert any("episode_digest_mismatch" in r for r in bridge.rejection_reasons)
    assert journal.entry_count == 0
...
def test_wrong_prior_journal_head_digest_rejects() -> None:
    journal = EvidenceJournal()
    bridge = _append(journal, expected_prior_journal_head_digest="b" * 64)
    assert bridge.status is PaperAdmissionLedgerBridgeStatus.REJECTED
    assert any("prior_journal_head_digest_mismatch" in r for r in bridge.rejection_reasons)
    assert journal.entry_count == 0


def test_stale_self_digest_alone_not_accepted_without_expected_anchor() -> None:
    # The caller's expected anchor is independent: a genuine self-digest paired with a WRONG expected anchor
    # is rejected (recompute == stored but stored != expected).
    journal = EvidenceJournal()
    bridge = _append(journal, expected_admission_digest="c" * 64)
    assert bridge.status is PaperAdmissionLedgerBridgeStatus.REJECTED
    assert any("admission_digest_mismatch" in r for r in bridge.rejection_reasons)
...
def test_episode_id_mismatch_rejects() -> None:
    journal = EvidenceJournal()
    bridge = _append(journal, episode_id="other-episode")
    assert bridge.status is PaperAdmissionLedgerBridgeStatus.REJECTED
...
def test_run_id_mismatch_rejects() -> None:
    journal = EvidenceJournal()
    bridge = _append(journal, run_id="other-run")
    assert bridge.status is PaperAdmissionLedgerBridgeStatus.REJECTED
    assert any("run_id_mismatch" in r for r in bridge.rejection_reasons)


def test_caller_correlation_mismatch_rejects() -> None:
    # The caller correlation differs from BOTH the episode runtime correlation and the record correlation.
    journal = EvidenceJournal()
    bridge = _append(journal, correlation_id="corr-other")
    assert bridge.status is PaperAdmissionLedgerBridgeStatus.REJECTED
...
def test_admission_record_correlation_mismatch_rejects() -> None:
    # Record points to a different correlation than the episode/caller -> "different run" fail-closed.
    record = _admitted_record(correlation_id="corr-adm-other")
    journal = EvidenceJournal()
    bridge = _append(journal, record=record)
    assert bridge.status is PaperAdmissionLedgerBridgeStatus.REJECTED
    assert any("admission_correlation_mismatch" in r for r in bridge.rejection_reasons)
    assert journal.entry_count == 0
...
def test_market_symbol_mismatch_rejects() -> None:
    record = _admitted_record(market_symbol="ETH-PERPETUAL")
    assert record.status is PaperEvidenceAdmissionStatus.ADMITTED
    journal = EvidenceJournal()
    bridge = _append(journal, record=record)
    assert bridge.status is PaperAdmissionLedgerBridgeStatus.REJECTED
...
def test_admission_not_admitted_rejects() -> None:
    ...
    journal = EvidenceJournal()
    bridge = _append(journal, record=rejected, expected_admission_digest=rejected.admission_digest)
    assert bridge.status is PaperAdmissionLedgerBridgeStatus.REJECTED
...
def test_episode_not_ready_rejects() -> None:
    rejected = _build_episode(expected_bridge_digest="d" * 64)
    assert rejected.status is PaperEndToEndEpisodeStatus.REJECTED
    journal = EvidenceJournal()
    bridge = _append(journal, episode=rejected, expected_episode_digest=rejected.episode_digest)
    assert bridge.status is PaperAdmissionLedgerBridgeStatus.REJECTED
...
def test_resealed_unsafe_admission_flag_rejects() -> None:
    tampered = replace(_admitted_record(), order_routed=True)
    sealed = replace(tampered, admission_digest=paper_evidence_admission_record_digest(tampered))
    journal = EvidenceJournal()
    bridge = _append(journal, record=sealed, expected_admission_digest=sealed.admission_digest)
    assert bridge.status is PaperAdmissionLedgerBridgeStatus.REJECTED
...
def test_resealed_unsafe_episode_flag_rejects() -> None:
    tampered = replace(_ready_episode(), live_api_called=True)
    sealed = replace(tampered, episode_digest=paper_end_to_end_episode_digest(tampered))
    journal = EvidenceJournal()
    bridge = _append(journal, episode=sealed, expected_episode_digest=sealed.episode_digest)
    assert bridge.status is PaperAdmissionLedgerBridgeStatus.REJECTED
...
def test_resealed_overclaim_episode_flag_rejects() -> None:
    tampered = replace(_ready_episode(), prdv4_stage4_complete=True)
    sealed = replace(tampered, episode_digest=paper_end_to_end_episode_digest(tampered))
    journal = EvidenceJournal()
    bridge = _append(journal, episode=sealed, expected_episode_digest=sealed.episode_digest)
    assert bridge.status is PaperAdmissionLedgerBridgeStatus.REJECTED
...
# --------------------------------------------------------------------------------------------------
# 5. Raw / canonical adversariality
# --------------------------------------------------------------------------------------------------
...
def test_equality_liar_admission_digest_cannot_bypass() -> None:
    record = replace(_admitted_record(), admission_digest=_LiarStr(_admitted_record().admission_digest))
    journal = EvidenceJournal()
    bridge = _append(journal, record=record, expected_admission_digest="a" * 64)
    assert bridge.status is PaperAdmissionLedgerBridgeStatus.REJECTED
    assert any("admission_digest_mismatch" in r for r in bridge.rejection_reasons)
    assert journal.entry_count == 0
...
# --------------------------------------------------------------------------------------------------
# 6. Fail-closed: call-level input
# --------------------------------------------------------------------------------------------------
...
def test_malformed_expected_admission_digest_raises(bad: str) -> None:
    ...
    with pytest.raises(PaperAdmissionLedgerBridgeError, match="expected_admission_digest_invalid"):
        _append(journal, expected_admission_digest=bad)
...
def test_malformed_prior_head_anchor_raises() -> None:
    ...
    with pytest.raises(PaperAdmissionLedgerBridgeError, match="expected_prior_journal_head_digest_invalid"):
        _append(journal, expected_prior_journal_head_digest="not-hex")
...
def test_empty_episode_id_raises(bad: str) -> None:
    ...
    with pytest.raises(PaperAdmissionLedgerBridgeError, match="episode_id_invalid"):
        _append(journal, episode_id=bad)


# --------------------------------------------------------------------------------------------------
# 7. Journal append / ledger semantics
# --------------------------------------------------------------------------------------------------
...
def test_duplicate_append_fails_closed_journal_unchanged() -> None:
    ...
    # Same record+episode -> identical curated payload -> journal payload_digest_replay -> fail-closed.
    second = _append(journal)
...
def test_append_to_non_empty_journal_chains() -> None:
    ...
    bridge = _append(journal, expected_prior_journal_head_digest=prior_head)
    assert bridge.ready is True
...
# --------------------------------------------------------------------------------------------------
# 8. Immutability / forbidden surfaces
# --------------------------------------------------------------------------------------------------
...
def test_module_purity_no_impure_imports() -> None:
    ...
    # The bridge legitimately reuses crypto_core.audit.evidence_journal (the existing store) and validation
    # artifacts; it must touch NO runtime/live/order/venue/service surface.
...
def test_no_forbidden_module_imports() -> None:
    # AST-based (robust to docstring prose, which legitimately documents what the module does NOT do): the
    # exact forbidden full module paths must never be imported. The module-purity test covers submodule/stdlib.
    tree = ast.parse(Path(bridge_module.__file__).read_text(encoding="utf-8"))
    imported: list[str] = []
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
