...
def test_validate_walk_forward_rejection_reasons_are_stable_and_ordered():
    result = validate_walk_forward(
        (
            _window("wf-1", trade_count=0, evidence_count=0),
            _window("wf-2", in_sample_sharpe=2.0, out_of_sample_sharpe=0.8),
            _window("wf-3", in_sample_hit_rate=65.0, out_of_sample_hit_rate=54.0),
        )
    )
    assert result.supportive is False
    assert result.rejection_reasons == (
