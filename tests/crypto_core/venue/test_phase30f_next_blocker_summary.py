...
def test_phase30f_summary_records_post_patch_outputs() -> None:
    text = SUMMARY.read_text(encoding="utf-8")
    readiness = evaluate_deribit_manual_review_readiness()
    gate_result = evaluate_deribit_normalized_book_quality(
...
def test_phase30f_summary_keeps_next_phase_bounded() -> None:
    text = SUMMARY.read_text(encoding="utf-8")

    assert "READY_FOR_HUMAN_GATE" in text
    assert "connector_ready_dialects_count" in text
    assert "still not trade-ready" in text.lower()
    assert "public feed ingest behind this quality gate" in text.lower()
