...
def test_phase71c_validation_fails_closed_on_connector_drift(monkeypatch) -> None:
    monkeypatch.setattr(
        "crypto_core.venue.deribit_paper_runtime_heartbeat_telemetry.connector_ready_dialects",
        lambda: (),
    )
    result = audit_deribit_paper_runtime_heartbeat_telemetry(
        json.loads(PHASE70.read_text(encoding="utf-8")),
    ...
    assert "deribit_paper_runtime_heartbeat_telemetry:connector_ready_dialects_mismatch" in result.rejection_reasons
