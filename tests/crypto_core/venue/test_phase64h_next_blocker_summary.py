...
from tests.crypto_core.venue.test_phase64b_runtime_enablement_approval_artifact import _approval

SUMMARY = Path("docs/crypto_core/DERIBIT_NEXT_BLOCKER_SUMMARY_64H.md")
...
def test_phase64h_next_blocker_summary_records_current_readiness_state() -> None:
...
def test_phase64h_next_blocker_summary_records_approval_without_runtime_start() -> None:
...
def test_phase64h_next_blocker_summary_points_to_execution_gate() -> None:
    text = _normalized_summary_text()

    assert "APPROVED_PAPER_RUNTIME_ENABLEMENT_EXECUTION_NOT_READY" in text
    assert "does not enable runtime" in text
