...
from tests.crypto_core.venue.test_phase74b_approved_paper_runtime_heartbeat_execution_artifact import _json

SUMMARY = Path("docs/crypto_core/DERIBIT_NEXT_BLOCKER_SUMMARY_74H.md")
...
def test_phase74h_summary_records_current_readiness_state() -> None:
...
def test_phase74h_summary_records_execution_state() -> None:
    text = _normalized_summary_text()
    artifact = _json(ARTIFACT)

    assert artifact["heartbeat_execution_status"] == "EXECUTED"
    for required in (
...
def test_phase74h_summary_points_to_next_blocker() -> None:
    text = _normalized_summary_text()
