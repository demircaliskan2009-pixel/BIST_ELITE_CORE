...
from tests.crypto_core.venue.test_phase71b_paper_runtime_heartbeat_telemetry_artifact import _json

SUMMARY = Path("docs/crypto_core/DERIBIT_NEXT_BLOCKER_SUMMARY_71H.md")
...
def test_phase71h_next_blocker_summary_records_current_readiness_state() -> None:
...
def test_phase71h_next_blocker_summary_records_runtime_heartbeat_telemetry_state() -> None:
    text = _normalized_summary_text()
    artifact = _json(ARTIFACT)

    assert artifact["heartbeat_telemetry_status"] == "PASS"
    for required in (
...
def test_phase71h_next_blocker_summary_points_to_operator_review_gate() -> None:
    text = _normalized_summary_text()

    assert "PAPER_RUNTIME_HEARTBEAT_OPERATOR_REVIEW_NOT_READY" in text
