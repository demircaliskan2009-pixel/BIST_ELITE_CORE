from __future__ import annotations

from pathlib import Path

from crypto_core.venue.contracts import VenueId
from crypto_core.venue.deribit_hard_capped_paper_session import DERIBIT_PAPER_SESSION_HARD_CAP
from crypto_core.venue.public_feed_dialects import connector_ready_dialects, dialects_for_venue
from tests.crypto_core.venue.test_phase47b_operator_approval_artifact import (
    _approval,
    _phase44_report_pack,
    _phase45_evaluation,
    _phase46_proposal,
)

PUBLIC_FEED_DIALECTS = Path("src/crypto_core/venue/public_feed_dialects.py")


def test_phase47f_hard_cap_and_existing_artifacts_are_not_changed() -> None:
    proposal = _phase46_proposal()
    evaluation = _phase45_evaluation()
    report_pack = _phase44_report_pack()
    approval = _approval()
    proposal_bounds = proposal["campaign_bounds"]
    approval_bounds = approval["campaign_bounds"]

    assert isinstance(proposal_bounds, dict)
    assert isinstance(approval_bounds, dict)
    assert DERIBIT_PAPER_SESSION_HARD_CAP == 3
    assert report_pack["hard_cap"] == evaluation["hard_cap"] == approval_bounds["hard_cap"] == 3
    assert proposal_bounds["hard_cap"] == approval_bounds["hard_cap"] == 3
    assert report_pack["per_session_max_trades"] == approval_bounds["per_session_max_trades"] == 2
    assert proposal_bounds["per_session_max_trades"] == approval_bounds["per_session_max_trades"] == 2
    assert report_pack["promotion_granted"] is False
    assert approval["promotion_granted"] is False


def test_phase47f_connector_ready_dialects_remains_one_deribit_public_dialect() -> None:
    ready = connector_ready_dialects()

    assert len(ready) == 1
    assert ready[0].venue_id is VenueId.DERIBIT


def test_phase47f_public_feed_registry_was_not_used_as_phase47_state_storage() -> None:
    text = PUBLIC_FEED_DIALECTS.read_text(encoding="utf-8")
    deribit = dialects_for_venue(VenueId.DERIBIT)[0]

    assert "phase47" not in text.lower()
    assert "approval_execution" not in text
    assert deribit.supports_checksum is False
    assert deribit.max_gap_tolerance == 0
    assert deribit.max_staleness_ns == 2_000_000_000
    assert deribit.max_receive_lag_ns == 1_000_000_000
