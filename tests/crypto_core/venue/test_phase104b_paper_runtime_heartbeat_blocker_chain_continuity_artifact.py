...
ARTIFACT = Path("docs/crypto_core/DERIBIT_PAPER_RUNTIME_HEARTBEAT_BLOCKER_CHAIN_CONTINUITY_104B.json")
...
def test_phase104b_artifact_schema_and_phase() -> None:
    art = _json(ARTIFACT)
    assert art["schema_version"] == "deribit_paper_runtime_heartbeat_blocker_chain_continuity.v1"
    assert art["phase"] == "104"


def test_phase104b_artifact_hash_matches_helper_output() -> None:
    art = _json(ARTIFACT)
...
def test_phase104b_artifact_parity_with_helper() -> None:
    result = audit_deribit_paper_runtime_heartbeat_blocker_chain_continuity_104(_json(P103))
    art = _json(ARTIFACT)
    for key in (
        "schema_version",
