...
from tests.crypto_core.venue.test_phase75b_paper_runtime_heartbeat_execution_telemetry_artifact import _json

SUMMARY = Path("docs/crypto_core/DERIBIT_NEXT_BLOCKER_SUMMARY_75H.md")
...
def test_phase75h_summary_records_current_readiness_state() -> None:
...
def test_phase75h_summary_records_telemetry_state() -> None:
    text = _normalized_summary_text()
    artifact = _json(ARTIFACT)

    assert artifact["heartbeat_execution_telemetry_status"] == "PASS"
    for required in (
...
def test_phase75h_summary_points_to_next_blocker() -> None:
    text = _normalized_summary_text()
