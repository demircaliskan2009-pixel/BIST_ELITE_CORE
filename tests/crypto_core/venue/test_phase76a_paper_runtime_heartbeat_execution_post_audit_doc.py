...
DOC = Path("docs/crypto_core/PAPER_RUNTIME_HEARTBEAT_EXECUTION_POST_AUDIT_76A.md")


def test_phase76a_doc_declares_post_audit_scope_and_boundary() -> None:
    text = DOC.read_text(encoding="utf-8")

    assert DOC.exists()
    assert "status: PAPER_RUNTIME_HEARTBEAT_EXECUTION_POST_AUDIT_COMPLETE" in text
    assert "scope: REPORT_ONLY_PAPER_RUNTIME_HEARTBEAT_EXECUTION_POST_AUDIT" in text
...
def test_phase76a_doc_declares_post_audit_metadata() -> None:
    text = DOC.read_text(encoding="utf-8")
...
def test_phase76a_doc_points_to_next_blocker() -> None:
    text = DOC.read_text(encoding="utf-8")
