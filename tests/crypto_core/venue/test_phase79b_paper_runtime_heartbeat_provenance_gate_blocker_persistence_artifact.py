...
def test_phase79b_artifact_schema_and_phase() -> None:
    art = _json(ARTIFACT)
    assert art["schema_version"] == "deribit_paper_runtime_heartbeat_provenance_gate_blocker_persistence.v1"
    assert art["phase"] == "79"


def test_phase79b_artifact_hash_matches_helper_output() -> None:
    art = _json(ARTIFACT)
...
def test_phase79b_artifact_parity_with_helper() -> None:
    r = audit_deribit_paper_runtime_heartbeat_provenance_gate_blocker_persistence(_json(P78))
    art = _json(ARTIFACT)
    for key in (
        "schema_version",
        "phase",
        "provenance_gate_blocker_persistence",
        "b5_status",
