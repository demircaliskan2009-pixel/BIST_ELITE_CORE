...
from tests.crypto_core.venue.test_phase42b_hard_capped_paper_session_artifact import _artifact, _run_phase42_session


def test_phase42f_artifact_contains_no_secret_endpoint_or_execution_material() -> None:
    payload = json.dumps({"artifact": _artifact(), "result": _run_phase42_session().artifact_payload}, sort_keys=True)
...
def test_phase42f_scope_flags_remain_disabled_and_safety_flags_remain_true() -> None:
    artifact = _artifact()
