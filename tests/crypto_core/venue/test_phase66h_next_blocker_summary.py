...
from tests.crypto_core.venue.test_phase66b_paper_runtime_start_proposal_artifact import _proposal
...
def test_phase66h_next_blocker_summary_records_current_readiness_state() -> None:
...
def test_phase66h_next_blocker_summary_records_start_proposal_state() -> None:
...
def test_phase66h_next_blocker_summary_points_to_runtime_start_approval_gate() -> None:
    text = _normalized_summary_text()

    assert "OPERATOR_PAPER_RUNTIME_START_APPROVAL_NOT_READY" in text
    assert "does not start runtime" in text
    assert "runtime remains enabled but not started" in text
