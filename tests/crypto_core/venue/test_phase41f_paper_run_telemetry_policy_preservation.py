...
from tests.crypto_core.venue.test_phase41b_paper_run_telemetry_report_artifact import _report, _run_artifact

PUBLIC_FEED_DIALECTS = Path("src/crypto_core/venue/public_feed_dialects.py")


def test_phase41f_phase40_artifact_bound_is_not_increased() -> None:
    run = _run_artifact()
    report = _report()

    assert run["max_trades"] == 1
    assert report["max_trades"] == 1
    assert report["next_blocker"] == "HARD_CAPPED_MULTI_RUN_SESSION_NOT_READY"


def test_phase41f_connector_ready_dialects_remains_one_deribit_public_dialect() -> None:
...
def test_phase41f_public_feed_registry_policy_values_are_preserved() -> None:
...
def test_phase41f_public_feed_registry_was_not_used_as_phase41_state_storage() -> None:
    text = PUBLIC_FEED_DIALECTS.read_text(encoding="utf-8")

    assert "phase41" not in text.lower()
    assert "telemetry_report" not in text
