...
class MatriksMarketDataProvider(MarketDataProvider):
    provider_name = "matriks"
    ...
    def latest_trading_day(self) -> str:
        raise FailClosedError(
            "Matriks market data provider contract is ready, but live wiring is deferred "
    ...
    def get_eod_range(
        ...
        if not self.api_key or not self.base_url:
            raise FailClosedError(
                "Matriks market data provider selected but BIST_MATRIKS_API_KEY or "
                "BIST_MATRIKS_BASE_URL is missing."
            )
        raise FailClosedError(
            "Matriks market data provider contract is ready, but live fetch wiring is deferred."
        )
