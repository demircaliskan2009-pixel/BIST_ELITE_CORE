...
def _ranked_item_view(item):
    ...
    rationale = _read(item, "rationale")
    if rationale is None:
        rationale = _read(item, "reason")
    if rationale is None:
        rationale = ""
    ...
    plan = _read(item, "plan") or {}
    if not isinstance(plan, dict):
        plan = {}

    current_close = _read(item, "current_close")
    if current_close is None:
        current_close = _read(item, "live_current_close")
    if current_close is None:
        current_close = plan.get("current_close")
    if current_close is None:
        current_close = plan.get("live_current_close")

    current_close_source = _read(item, "current_close_source")
    if current_close_source is None:
        current_close_source = _read(item, "live_current_close_source")
    if current_close_source is None:
        current_close_source = plan.get("current_close_source")
    if current_close_source is None:
        current_close_source = plan.get("live_current_close_source")

    live_vs_g_close_pct = _read(item, "live_vs_g_close_pct")
    if live_vs_g_close_pct is None:
        live_vs_g_close_pct = plan.get("live_vs_g_close_pct")

    entry_status = _read(item, "entry_status")
    if entry_status is None:
        entry_status = plan.get("entry_status")

    out = {
        "symbol": _read(item, "symbol"),
        "score": _read(item, "score"),
        "rationale": rationale,
    }
    if current_close is not None:
        out["current_close"] = current_close
    if current_close_source is not None:
        out["current_close_source"] = current_close_source
    if live_vs_g_close_pct is not None:
        out["live_vs_g_close_pct"] = live_vs_g_close_pct
...
def _cmd_eod_execute(args: argparse.Namespace) -> int:
    ...
    # SAFE_LIVE_OPTIN_V4: keep --execution live as LIVE (fail-closed on missing broker-config); --live only arms sending (dry_run=False)
    armed_live = bool(getattr(args, "live", False))
    broker_name = (
    ...
    live = execution == "live"
    dry_run = bool(getattr(args, "dry_run", False)) or (live and not armed_live)
    if not day or not str(outdir):
        raise SystemExit("--day and --outdir are required")
    day_dir = outdir / day
    day_dir.mkdir(parents=True, exist_ok=True)
...
def _cmd_ask_legacy(args: argparse.Namespace) -> int:
...
def _build_scan_artifact(day_str: str, ranked) -> dict[str, object]:
    lines = [_scan_ranked_item_view(r, day_str) for r in ranked]
    return {
...
def _extract_symbols_from_freeform(text: str, allowed_symbols):
    ...
    seen = set()
    for tok in toks:
        if tok in allowed_symbols and tok not in seen:
            out.append(tok)
            seen.add(tok)
    return out
...
def _emit_freeform_response(args, day: str, query_text: str, payload: dict) -> int:
    ...
    base = Path(getattr(args, "out", None) or "data/out/ask")
    out_dir = base / str(day)
    out_dir.mkdir(parents=True, exist_ok=True)
    art = out_dir / f"FREEFORM_{_freeform_slug(query_text)[:96]}.json"
...
def _handle_freeform_single_pick(args, query_text: str) -> int:
    ...
    snapshot_root = _resolve_snapshot_root_for_freeform(args)
    tmp_out = Path("data/out/freeform_scan")
    ...
    for rec in (result.get("records") or [])[:3]:
        sym = rec.get("symbol")
        score = rec.get("score")
        reason = rec.get("reason") or rec.get("rationale") or ""
        ranked.append(_scan_ranked_item_view((sym, score, reason), str(day)))

    if not ranked:
        raise SystemExit("Geçerli aday bulunamadı.")

    best = ranked[0]

    detail_bits = []
    if best.get("entry_status") is not None:
        detail_bits.append(f"durum={best['entry_status']}")
    if best.get("entry_gap_pct") is not None:
        detail_bits.append(f"entry_gap={float(best['entry_gap_pct']):+.2f}%")
    if best.get("current_close") is not None:
        live_txt = f"canlı={best['current_close']}"
        if best.get("current_close_source"):
            live_txt += f" ({best['current_close_source']})"
        detail_bits.append(live_txt)
    ...
    payload = {
        "query_type": "single_pick",
        "day": str(day),
        "query": query_text,
        "best_symbol": best["symbol"],
        "ranked": ranked,
        "text": (
            f"{best['symbol']} bugün relatif olarak en güçlü aday. "
            f"Skor {float(best['score']):.2f}. "
            f"Kompakt gerekçe: {best['rationale']}."
            f"{detail_txt}"
        ),
    }
...
def _handle_freeform_compare(args, query_text: str, symbols: list[str]) -> int:
    ...
    rows = sorted(rows, key=lambda x: (-float(x["score"]), x["symbol"]))
    best = rows[0]
...
def _cmd_ask(args):
    ...
    if not query_text or not _looks_like_freeform_ask_query(query_text):
        return _cmd_ask_legacy(args)
    ...
    snapshot_root = _resolve_snapshot_root_for_freeform(args)
    allowed = _snapshot_symbols_for_day(snapshot_root, str(day))
    if not allowed:
        return _cmd_ask_legacy(args)

    symbols = _extract_symbols_from_freeform(query_text, allowed)
    ql = query_text.lower()

    if len(symbols) >= 2:
        return _handle_freeform_compare(args, query_text, symbols)

    if any(k in ql for k in ("tek hisse", "hangi hisse", "hangisi olurdu", "en güçlü")):
        return _handle_freeform_single_pick(args, query_text)

    if len(symbols) == 1:
        if getattr(args, "symbol_opt", None):
            args.symbol_opt = symbols[0]
        else:
...
def _scan_ranked_item_view(item, day_str: str | None = None):
    ...
    def _merge_live_fields(out: dict, plan: dict | None):
        ...
        if current_close_source is None:
            current_close_source = plan.get("live_current_close_source")
        if current_close is not None and current_close_source is None:
            current_close_source = "snapshot"

        live_vs_g_close_pct = plan.get("live_vs_g_close_pct")
        entry_status = plan.get("entry_status")
        entry_gap_pct = plan.get("entry_gap_pct")

        if current_close is not None and "current_close" not in out:
            out["current_close"] = current_close
        if current_close_source is not None and "current_close_source" not in out:
            out["current_close_source"] = current_close_source
        if live_vs_g_close_pct is not None and "live_vs_g_close_pct" not in out:
            out["live_vs_g_close_pct"] = live_vs_g_close_pct
        if entry_status is not None and "entry_status" not in out:
            out["entry_status"] = entry_status
        if entry_gap_pct is not None and "entry_gap_pct" not in out:
            out["entry_gap_pct"] = entry_gap_pct
        return out

    def _compact_rationale(signals: dict | None, plan: dict | None, fallback: str):
        signals = signals if isinstance(signals, dict) else {}
        plan = plan if isinstance(plan, dict) else {}

        status = plan.get("entry_status")
        gap = signals.get("entry_gap_pct")
        if gap is None:
            gap = plan.get("entry_gap_pct")

        comps = signals.get("score_components")
        ...
        if gap_num is not None:
            parts.append(f"gap={gap_num:+.2f}%")

        comp_bits = []
        for key, label in (("entry", "E"), ("ret1", "R1"), ("range", "Rp"), ("volume", "V")):
        ...
        if parts:
            return _trim_rationale("; ".join(parts))

        return _trim_rationale(fallback)

    if isinstance(item, (tuple, list)):
        symbol = item[0] if len(item) > 0 else None
        score = item[1] if len(item) > 1 else None
        fallback_rationale = item[2] if len(item) > 2 else ""

        out = {
            "symbol": symbol,
            "score": score,
            "rationale": _trim_rationale(fallback_rationale),
        }

        if symbol and day_str:
            try:
                import os
                from pathlib import Path
                ...
                advice = build_advice_for_symbol(str(symbol), str(day_str), root=root)

                signals = advice.signals if isinstance(getattr(advice, "signals", None), dict) else {}
                plan = advice.plan if isinstance(getattr(advice, "plan", None), dict) else {}

                out["rationale"] = _compact_rationale(signals, plan, fallback_rationale)
                out = _merge_live_fields(out, plan)
            except Exception:
                pass

        return out

    fallback_rationale = _read(item, "rationale")
    if fallback_rationale is None:
        fallback_rationale = _read(item, "reason")

    signals = _read(item, "signals")
    signals = dict(signals) if isinstance(signals, dict) else {}

    plan = _read(item, "plan")
    plan = dict(plan) if isinstance(plan, dict) else {}

    out = {
        "symbol": _read(item, "symbol"),
        "score": _read(item, "score"),
        "rationale": _compact_rationale(signals, plan, fallback_rationale),
    }
    ...
    entry_gap_pct = _read(item, "entry_gap_pct")
