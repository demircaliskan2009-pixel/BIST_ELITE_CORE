...
def generate_advice(
    ...
    *,
        ...
        for symbol in symbols:
            ...
            try:
                ...
                if isinstance(advice.plan, dict):
                    rec["current_close"] = advice.plan.get("current_close")
                    rec["current_close_source"] = advice.plan.get("live_current_close_source")
                    rec["live_vs_g_close_pct"] = advice.plan.get("live_vs_g_close_pct")
                    rec["entry_status"] = advice.plan.get("entry_status")
            except Exception:
    ...
    if top_n is not None and top_n > 0 and records:
        # top_n çıktısında ranking sırasını koru; alfabetik resort ranking'i bozmasın
        records = sorted(records, key=lambda r: (-float(r["score"]), r["symbol"]))[:top_n]
    else:
        # tam advice dump için deterministik alfabetik sıralama korunur
