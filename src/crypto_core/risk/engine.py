"""Risk Engine v1 + v2 — fail-closed approval gate (PRD §1.14–§1.28).

v1 evaluate() — 4-gate chain (unchanged, fully backward compatible).
v2 evaluate_v2() — extends v1 with 5 additional gates:
  Gate 5: kill-switch level enforcement (§1.19)
  Gate 6: DTL / distance-to-liquidation enforcement (§1.26)
  Gate 7: Kelly criterion sizing contract (§1.28)
  Gate 8: CVaR limit check (§1.18)
  Gate 9: portfolio-level exposure / leverage limits

All gates fail-closed: any unhandled exception → BLOCKED.
...
#: System-wide maximum leverage (PRD invariant: 3×)
...
class RiskEngine:
    ...
    def evaluate_v2(self, risk_input: RiskInput) -> RiskEvaluation:
        """Full v2 evaluation — extends v1 with KS, DTL, Kelly, CVaR, portfolio gates.
        ...
        try:
            ...
            logger.exception("RiskEngine.evaluate_v2 raised — fail-closed BLOCKED")
            return RiskEvaluation(
                decision=RiskDecision.BLOCKED,
                block_reason=RiskBlockReason.EXCEPTION_FAIL_CLOSED,
    ...
    def _gate_kill_switch(
        ...
        """Gate 5: kill-switch level enforcement (PRD §1.19)."""
    ...
    def _gate_dtl(
        ...
        """Gate 6: Distance-to-Liquidation enforcement (PRD §1.26).
        ...
        # liq_price=0 → data unavailable → skip gate, note in evidence
    ...
    def _gate_kelly(
        ...
        """Gate 7: Kelly criterion sizing (PRD §1.28).
        ...
        # Validate inputs first — invalid inputs block (fail-closed)
        ...
        if f_star <= 0.0:
            # Non-positive Kelly: edge has no positive expected value → block
    ...
    def _gate_cvar(
        ...
        """Gate 8: CVaR limit check (PRD §1.18).
    ...
    def _gate_portfolio(
        ...
        """Gate 9: Portfolio-level limits (PRD §1.26).
        ...
        # Hard cap: system-wide 3× leverage invariant (PRD invariant)
