...
def _result(window_id: str, valid: bool, supportive: bool, *reasons: str) -> WalkForwardWindowResult:
    return WalkForwardWindowResult(window_id, valid, supportive, tuple(reasons))
...
def _evaluate_window(
    ...
    if not isinstance(window, WalkForwardWindow):
        return _result(f"window[{index}]", False, False, _reason(index, "malformed"))
    reasons: list[str] = []
    ...
    if reasons:
        return _result(window.window_id, False, False, *reasons)
    ...
    return _result(window.window_id, True, not reasons, *reasons)
...
def validate_walk_forward(
    ...
    *,
    ...
    if windows is None:
        return WalkForwardValidationResult(False, 0, 0, 0, ("windows_missing",), ())
    try:
        ordered_windows = tuple(windows)
    except TypeError:
        return WalkForwardValidationResult(False, 0, 0, 0, ("windows_unreadable",), ())
    if not ordered_windows:
        return WalkForwardValidationResult(False, 0, 0, 0, ("windows_empty",), ())
    ...
    return WalkForwardValidationResult(
        supportive=not rejection_reasons and valid_oos_window_count >= min_oos_windows,
