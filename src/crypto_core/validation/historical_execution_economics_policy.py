"""Governed execution-economics policy for deterministic historical execution (DETERMINISTIC_HISTORICAL_EXECUTION_ECONOMICS_V2).
...
Numeric discipline (``HISTORICAL_EXECUTION_NUMERIC_POLICY_V1``, committed by the digest) draws one explicit line, and
it is the line this contract is built on:

* ``state_representation_id`` — internal accounting state (position quantity, cost basis, realized, fees, funding) is
  EXACT ``Fraction`` state. Rendering to scale-18 text is ARTIFACT OUTPUT ONLY: no rendered field is ever converted
  back into internal accounting authority (``EXACT_INTERNAL_COST_BASIS_NEVER_ROUNDED_FOR_STATE``). A rendered
  average entry price is therefore a view of exact state, never the state itself;
* ``executed_price_grid_id`` and ``quantity_rounding_id`` — the only two values quantized ONCE as an economic rule are
  the executed price (ROUND_HALF_EVEN onto the scale-18 price grid) and the executable quantity (truncated toward
  zero). Both are quantized by exact integer arithmetic that yields the exact grid value and its canonical text
  together, so the artifact text and the internal value are the same number and nothing is re-parsed;
* ASCII fixed scale-18 decimals, no exponent, plus sign or negative zero, at most 60 characters; native integers
  bounded to int64 before serialization; exact ``Fraction`` arithmetic throughout. There is no ``decimal`` context
  anywhere and no float.
...
# --- contract identities (committed by the policy digest) ------------------------------------------------------------
...
FUNDING_COVERAGE_RULE_V1 = "every_governed_grid_liability_of_every_liable_interval_v1"
...
ACCOUNTING_STATE_RULE_V1 = "exact_signed_quantity_and_cost_basis_v1"
...
class HistoricalExecutionConformance:
    """Digest-bound PRDV4 conformance declaration of the execution model; false fields are never hidden."""
    ...
    exact_internal_cost_basis_enforced: bool
...
def historical_execution_decimal_is_canonical(value: object) -> bool:
    """Canonical ASCII fixed-scale decimal within the representation-safety length of the numeric policy."""
...
def _half_even_units(value: Fraction) -> tuple[bool, int]:
    """``(negative, scale-18 unit count)`` of ``value`` under exact integer ROUND_HALF_EVEN."""
    ...
    if twice > magnitude.denominator or (twice == magnitude.denominator and quotient % 2 == 1):
        quotient += 1
    return value < 0, quotient


def _truncated_units(value: Fraction) -> tuple[bool, int]:
    """``(negative, scale-18 unit count)`` of ``value`` truncated toward zero (never exceeds the exact magnitude)."""

    magnitude = abs(value)
    return value < 0, (magnitude.numerator * _SCALE_FACTOR) // magnitude.denominator


def _units_text(negative: bool, units: int) -> str | None:
...
def _units_value(negative: bool, units: int) -> Fraction:
    return Fraction(-units if negative else units, _SCALE_FACTOR)


def historical_execution_render_amount(value: Fraction) -> str | None:
    """Exact ROUND_HALF_EVEN render at the policy scale; ``None`` when outside the representation domain.

    Rendering is ARTIFACT OUTPUT ONLY. No caller converts the returned text back into internal accounting state.
    """

    return _units_text(*_half_even_units(value))


def historical_execution_render_quantity(value: Fraction) -> str | None:
    """Render truncated toward zero at the policy scale; ``None`` when outside the representation domain."""

    return _units_text(*_truncated_units(value))


def historical_execution_quantize_price(value: Fraction) -> tuple[Fraction, str] | None:
    """The executed-price grid rule: ``(exact grid value, its canonical text)``, or ``None`` when unrepresentable.

    Both members come from ONE exact integer unit count, so the emitted text and the value used by the economics are
    the same number and no text is ever re-parsed into state.
    """

    negative, units = _half_even_units(value)
    text = _units_text(negative, units)
    return None if text is None else (_units_value(negative, units), text)


def historical_execution_quantize_quantity(value: Fraction) -> tuple[Fraction, str] | None:
    """The executable-quantity grid rule (truncation toward zero), as ``(exact grid value, its canonical text)``."""

    negative, units = _truncated_units(value)
    text = _units_text(negative, units)
    return None if text is None else (_units_value(negative, units), text)
...
def _assemble_policy(
    *,
    ...
    seed = HistoricalExecutionEconomicsPolicy(
        ...
        funding_coverage_rule_id=FUNDING_COVERAGE_RULE_V1,
        ...
        accounting_state_rule_id=ACCOUNTING_STATE_RULE_V1,
